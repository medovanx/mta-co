...
namespace MTA.Client.Commands
{
    public static class GMCommands
    {
        public static bool CheckCommand(GameState client, string message)
        {
            try
            {
                // Message already verified to start with "@" by PacketHandler.cs
                ...
                if (MessageTestCommands.HandleCommand(client, Data, Mess))
                    return true;
                if (MonsterTestCommands.HandleCommand(client, Data, Mess))
                    return true;
                ...
                // Try Event commands
                if (EventCommands.HandleCommand(client, Data, Mess))
                    return true;
