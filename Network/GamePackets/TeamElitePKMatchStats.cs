...
namespace MTA.Network.GamePackets {
    public class TeamElitePKMatchStats : Writer, IPacket {
        byte[] Buffer;
        ...
        private void AppendPlayer(TeamElitePk.FighterStats team, ushort offset) {
            WriteUint(team.LeaderUID, offset, Buffer);
            offset += 4;
            WriteUint(team.Team.UID, offset, Buffer);
            offset += 4;
            WriteString(team.Name, offset, Buffer);
            offset += 32; //???
