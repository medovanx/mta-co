...
namespace MTA.Network.GamePackets {
    public class PerfectionTable {
        ...
        public static uint PerfectionPoints(GameState client, bool Garment) {
            uint Count = 0;
        ...
        public static uint AmountStarMounts(GameState client, byte Star) {
            uint Count = 0;
    ...
    public class Proto {
        public static byte[] FinalizeProtoBuf(object Pro, ushort ID) {
            ...
            using (var ms = new MemoryStream()) {
                ...
                Writer.Write(ID, 2, buffer);
                return buffer;
            }
        }

        public static void Pro(ConquerItem I, GameState C) {
            var P = FinalizeProtoBuf((new PerfectionItems.Item() {
                IUID = I.UID,
                EUID = C.Entity.UID,
                OUID = I.OwnerID,
                OName = I.OwnerName,
    ...
    public sealed class PerfectionGUI {
        ...
        public void Handle(GameState client) {
            switch (Info.ID) {
                case 0: {
        ...
        public class PerfectionProto {
            [ProtoMember(1, IsRequired = true)] public uint ID;

            [ProtoMember(2, IsRequired = true)] public uint LevelMax;
        }
    }
    ...
    public sealed class PerfectionItems {
        ...
        public void Handle(byte[] Packet, GameState C) {
            try {
                switch (Info.ID) {
                    #region Temper
                    ...
                    case 0: {
                        ...
                        if (C.Equipment.TryGetItem(Info.ItemUID, out Item) &&
                            C.Inventory.TryGetItem(Info.ItemPlusUID, out ItemPlus)) {
                            if (ItemPlus.ID == 3009000 || ItemPlus.ID == 3009001 || ItemPlus.ID == 3009002 ||
                                ItemPlus.ID == 3009003) {
                                ...
                                while (Item.PerfectionProgress >= GetProgress(Item)) {
                                    ...
                                    Kernel.SendWorldMessage(
                                        new Message(
                                            "Congratulations, " + C.Entity.Name + " has upgraded Perfection " +
                                            ConquerItemInformation.BaseInformations[Item.ID].Name + " to level + " +
                                            Item.Perfectionlevel + " !", Color.Red, Message.TopLeft), Program.Values);
                                }
                                ...
                                Proto.Pro(Item, C);
                                C.LoadItemStats();
                                C.Inventory.Remove(ItemPlus.UID, Enums.ItemUse.Remove, true);
                    ...
                    case 4: {
                        ...
                        if (C.Equipment.TryGetItem(Info.ItemUID, out Item) ||
                            C.Inventory.TryGetItem(Info.ItemUID, out Item)) {
                            if (C.Entity.ConquerPoints >= 1000) {
                                UsedItems = new ConquerItem[Info.ItemPlusUID];
                                for (var i = 0; i < UsedItems.Length; i++) {
                                    if (!C.Inventory.TryGetItem(Info.ItemPlusUID, out UsedItems[i]) &&
                                        !C.Equipment.TryGetItem(Info.ItemPlusUID, out UsedItems[i])) {
                                        return;
                                    }
                                }
                                ...
                                Item.Mode = Enums.ItemMode.Update;
                                C.Send(Item);
                                UsedItems[0].Mode = Enums.ItemMode.Update;
                                C.Send(UsedItems[0]);
                                Proto.Pro(Item, C);
                                ConquerItemTable.UpdatePerfection(UsedItems[0]);
                                ConquerItemTable.UpdatePerfection(Item);
                            }
                        }
        ...
        public class GUI {
            ...
            [ProtoMember(4, IsRequired = true)] public uint ItemPlusUID;

            [ProtoMember(2, IsRequired = true)] public uint ItemUID;

            [ProtoMember(3, IsRequired = true)] public string Signature;
        }

        [ProtoContract]
        public class Item {
            [ProtoMember(2, IsRequired = true)] public uint EUID;

            [ProtoMember(1, IsRequired = true)] public uint IUID;

            [ProtoMember(3, IsRequired = true)] public uint Level;

            [ProtoMember(6, IsRequired = true)] public string OName;

            [ProtoMember(5, IsRequired = true)] public uint OUID;

            [ProtoMember(4, IsRequired = true)] public uint Progress;
        }
    }
    ...
    public sealed class PerfectionEffect {
        ...
        public static void Send(GameState C, uint E) {
        ...
        public class PerfectionE {
            [ProtoMember(3, IsRequired = true)] public uint Eff;

            [ProtoMember(2, IsRequired = true)] public uint Type;
    ...
    public sealed class PerfectionScore {
        public static List<Entity> RankingList;
        ...
        public void GetRankingList() {
            ...
            using (var reader = new MySqlReader(cmd)) {
                while (reader.Read()) {
                    ...
                    Entity client = new Entity(EntityFlag.Player, false);
                    ...
                    RankingList.Add(Kernel.GamePool.ContainsKey(client.UID)
                        ? Kernel.GamePool[client.UID].Entity
                        : client);
                }
            }
        }
        ...
        public bool Read(byte[] packet) {
            try {
                ...
                using (var memoryStream = new MemoryStream(packet)) {
                    Info = Serializer.DeserializeWithLengthPrefix<PerfectionScoreProto>(memoryStream,
                        PrefixStyle.Fixed32);
                }
            }
            catch (Exception e) {
        ...
        public void SendScore(GameState client, GameState Observer) {
        ...
        public uint GetScoreValue(GameState client, uint Type) {
            uint Score = 0;
            if (Type == 2) Score = (uint)CalculatePerfectionChiPoints(client);
            if (Type == 4) Score = client.Equipment.GetFullEquipmentEnchantPoints;
            if (Type == 5)
                Score = (uint)(client.Entity.Level < 140 ? client.Entity.Level * 20 : client.Entity.Level * 25);
            if (Type == 6) Score = client.Equipment.GetFullEquipmentPerfecetionLevelPoints;
            if (Type == 7)
                Score = (uint)((client.Entity.Vitality + client.Entity.Atributes + client.Entity.Spirit +
                                client.Entity.Strength + client.Entity.Agility) * 5);
        ...
        public static int CalculatePerfectionChiPoints(GameState client) {
            int Point = 0;
            if (client.ChiData != null) {
                var chiScores = new int[] {
                    client.ChiData.DragonPoints, client.ChiData.PhoenixPoints, client.ChiData.TigerPoints,
                    client.ChiData.TurtlePoints
                };
        ...
        public static uint CalculateSubClassPoints(GameState client) {
            uint Points = 0;
        ...
        public class PerfectionScoreProto {
            [ProtoMember(1, IsRequired = true)] public uint ActionId;

            [ProtoMember(3, IsRequired = true)] public uint Level;

            [ProtoMember(4, IsRequired = true)] public Scores[] Scores;

            [ProtoMember(2, IsRequired = true)] public uint UID;
        }

        [ProtoContract]
        public class Scores {
            [ProtoMember(2, IsRequired = true)] public uint Score;

            [ProtoMember(1, IsRequired = true)] public uint Type;
        }
    }
    ...
    public sealed class PerfectionRank {
        ...
        public void UpdateRanking() {
            ...
            for (byte i = 0; i < 11; i++) {
                ...
                MainRank[z] = items.OrderByDescending(x => PerfectionScore.CalculatePerfectionItemPoints(x))
                    .FirstOrDefault();
            }
        }
        ...
        public void Handle(GameState client) {
            switch (Info.Action) {
                case ActionID.UserItemRanking: {
                    var item = AllItemsRanking.Where(p => p.Position == GetRealyPosition((byte)Info.Index))
                        .OrderByDescending(i => PerfectionScore.CalculatePerfectionItemPoints(i)).ToArray();
                ...
                case ActionID.MyRanks: {
                    ...
                    for (byte i = 0; i < packet.items.Length; i++) {
                        byte z = GetRealyPosition((byte)(i + 1));
                        var itemmm = AllItemsRanking.Where(p => p.Position == z)
                            .OrderByDescending(m => PerfectionScore.CalculatePerfectionItemPoints(m)).ToArray();
                ...
                {
                    if (ConquerItemTable.LoadItem(Info.Index) != null) {
                        var item = ConquerItemTable.LoadItem(Info.Index);
                        item.Mode = Enums.ItemMode.PerfectionView;
                        item.Send(client);
                    }
                    else {
                ...
                case ActionID.RankItems: //AllRanking
                {
                    ...
                    for (int i = rank; i < rank + sss; i++) {
                        var iteeeem = AllItemsRanking.Where(p => p.Position == GetRealyPosition((byte)Info.Index))
                            .OrderByDescending(x => PerfectionScore.CalculatePerfectionItemPoints(x)).ToArray()[i];
                        packet.items[i] = new EquipProto();
                        if (iteeeem.ID > 0) {
        ...
        public class PerfectionRankProto {
            [ProtoMember(1, IsRequired = true)] public ActionID Action;

            [ProtoMember(4, IsRequired = true)] public uint Index;

            [ProtoMember(5, IsRequired = true)] public EquipProto[] items;

            [ProtoMember(3)] public uint Page;

            [ProtoMember(2)] public uint RegisteredCount;
        }

        [ProtoContract]
        public class EquipProto {
            [ProtoMember(6, IsRequired = true)] public uint ItemID;

            [ProtoMember(10, IsRequired = true)] public string Name = "";

            [ProtoMember(9, IsRequired = true)] public uint PerfectionLevel;

            [ProtoMember(8, IsRequired = true)] public uint Plus;

            [ProtoMember(3, IsRequired = true)] public uint Position;

            [ProtoMember(7, IsRequired = true)] public uint PurificationID;

            [ProtoMember(1, IsRequired = true)] public uint Rank;

            [ProtoMember(4, IsRequired = true)] public uint RankScore;

            [ProtoMember(5, IsRequired = true)] public uint UID;

            [ProtoMember(2, IsRequired = true)] public uint UnKnow2;
        }
    }
    ...
    class PrestigeRank {
        ...
        public static ConquerItem LoadItem(uint OwnerUId, uint Position) {
            ...
            using (var cn = new MySqlConnection(DataHolder.MySqlConnection.ConnectionString))
            using (var cm = new MySql.Data.MySqlClient.MySqlCommand(
                       "SELECT * FROM items WHERE EntityID = @u AND Position =@p", cn)) {
                cn.Open();
                cm.Parameters.AddWithValue("@u", OwnerUId);
                cm.Parameters.AddWithValue("@p", Position);
                using (MySqlDataReader rdr = cm.ExecuteReader())
                    if (rdr.Read())
        ...
        public static ConquerItem deserialzeItem(MySqlDataReader reader) {
            ConquerItem item = new ConquerItem(true);
            ...
            item.SocketOne = (Enums.Gem)reader.GetUInt16("SocketOne");
            item.SocketTwo = (Enums.Gem)reader.GetUInt16("SocketTwo");
            item.Effect = (Enums.ItemEffect)reader.GetUInt16("Effect");
            item.Mode = Enums.ItemMode.Default;
            ...
            item.Color = (Enums.Color)reader.GetUInt32("Color");
        ...
        public static void Load() {
            SR.Clear();
            Topers.Clear();
            using (MySqlConnection conn = new MySqlConnection(DataHolder.MySqlConnection.ConnectionString)) {
                conn.Open();
                uint TopUid = 0;
                uint TopScore = 0;
                foreach (var C in Classs) {
                    using (MySql.Data.MySqlClient.MySqlCommand cmd =
                           new MySql.Data.MySqlClient.MySqlCommand(
                               "select TotalPerfectionScore,Body,Name,Class,Level,UID,Face from entities WHERE class =@c order by TotalPerfectionScore desc limit 30 ")) {
                ...
                if (TopUid != 0) {
                    ...
                    B.DB.Rank = 1;
                    using (MySql.Data.MySqlClient.MySqlCommand cmd =
                           new MySql.Data.MySqlClient.MySqlCommand("SELECT * FROM entities WHERE UID = @u", conn)) {
        ...
        [ProtoContract]
        public class Best {
            [ProtoMember(2, IsRequired = true)] public DataBest DB;
        ...
        public class DataBest {
            [ProtoMember(3, IsRequired = true)] public uint EntityUID;

            [ProtoMember(15, IsRequired = true)] public uint Flag;

            [ProtoMember(19, IsRequired = true)] public uint Flag1;

            [ProtoMember(20, IsRequired = true)] public uint Flag2;

            [ProtoMember(9, IsRequired = true)] public uint Garment;

            [ProtoMember(5, IsRequired = true)] public string GuildName = "";

            [ProtoMember(7, IsRequired = true)] public uint HairStyle;

            [ProtoMember(8, IsRequired = true)] public uint Head;

            [ProtoMember(10, IsRequired = true)] public uint LeftWeapon;

            [ProtoMember(11, IsRequired = true)] public uint LeftWeaponAccessory;

            [ProtoMember(6, IsRequired = true)] public uint Mesh;

            [ProtoMember(14, IsRequired = true)] public uint MountArmor;

            [ProtoMember(4, IsRequired = true)] public string Name = "";

            [ProtoMember(2, IsRequired = true)] public uint Rank; //1

            [ProtoMember(12, IsRequired = true)] public uint RightWeapon;

            [ProtoMember(13, IsRequired = true)] public uint RightWeaponAccessory;

            [ProtoMember(18, IsRequired = true)] public uint Title;

            [ProtoMember(1, IsRequired = true)] public uint Type; //1

            [ProtoMember(16, IsRequired = true)] public uint Wing;

            [ProtoMember(17, IsRequired = true)] public uint WingPlus;
        }
    }
}