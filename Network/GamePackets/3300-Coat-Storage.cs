...
namespace MTA.Network.GamePackets {
    public sealed class CoatStorage {
                ...
                case Action.Addtowardrobe:
                    ...
                    if (client.Inventory.TryGetItem(Info.ItemGuid, out myItem) ||
                        ...
                        if (!client.Entity.StorageItems.ContainsKey(myItem.UID)) {
                            ...
                            ConquerItemTable.UpdateWardrobe(myItem.InWardrobe, myItem.UID);
                        }
                        ...
                        foreach (var i in client.Entity.StorageItems.Values)
                            if (i.Position != 0 && i.Position == (byte)PacketHandler.GetPositionFromID(myItem.ID)) {
                                i.Position = 0;
                                ConquerItemTable.UpdateLocation(i, client);
                            }

                        myItem.Position = (byte)PacketHandler.GetPositionFromID(myItem.ID);

                        client.Inventory.Remove(myItem, Enums.ItemUse.Remove);
                        ...
                        if ((equips.Garment == 0 || equips.Garment > 0 && equips.Garment != myItem.UID) &&
                            myItem.Position != 17)
                            equips.Garment = myItem.UID;

                        client.Send(equips.ToArray());

                        ConquerItemTable.UpdateLocation(myItem, client);
                ...
                case Action.Retrieve:
                    if (client.Entity.StorageItems.TryGetValue(Info.ItemGuid, out myItem)) {
                        ...
                        if (client.Inventory.ContainsUID(myItem.UID)) {
                            client.Inventory.Remove(myItem, Enums.ItemUse.Move);
                            if (client.Equipment.TryGetItem(pos) != null &&
                                client.Equipment.TryGetItem(pos).UID == myItem.UID)
                                client.Equipment.Remove(pos);
                            else client.Inventory.Add(myItem, Enums.ItemUse.Move);
                        }
                        else {
                            if (client.Equipment.TryGetItem(pos) != null &&
                                client.Equipment.TryGetItem(pos).UID == myItem.UID)
        ...
        private bool Illegal(CoatStorageProto Info, GameState client) {
            ConquerItem myItem;
            if (client.Inventory.TryGetItem(Info.ItemGuid, out myItem) ||
                client.Entity.StorageItems.TryGetValue(Info.ItemGuid, out myItem) ||
                client.Equipment.TryGetItem(Info.ItemGuid, out myItem)) {
                var dbInfo = ConquerItemInformation.BaseInformations.ContainsKey(myItem.ID)
                    ? ConquerItemInformation.BaseInformations[myItem.ID]
                    : null;
                if (dbInfo == null)
                    return true;
        ...
        public void Login(GameState client) {
            ...
            if (currentGarment != null && !client.Entity.StorageItems.ContainsKey(currentGarment.UID)) {
                ...
                client.Send(FinalizeProtoBuf(pkt));
                ConquerItemTable.UpdateWardrobe(currentGarment.InWardrobe, currentGarment.UID);
            }
            ...
            if (currentMountArmor != null && !client.Entity.StorageItems.ContainsKey(currentMountArmor.UID)) {
                ...
                client.Send(FinalizeProtoBuf(pkt));
                ConquerItemTable.UpdateWardrobe(currentMountArmor.InWardrobe, currentMountArmor.UID);
            }
    ...
    public class CoatStorageProto {
        [ProtoMember(1, IsRequired = true)] public CoatStorage.Action ActionId;

        [ProtoMember(5, IsRequired = true)] public ItemStorage Item;

        [ProtoMember(2, IsRequired = true)] public uint ItemGuid;

        [ProtoMember(3, IsRequired = true)] public int ItemId;

        [ProtoMember(4, IsRequired = true)] public int Junk;
    ...
    public class ItemStorage {
        [ProtoMember(10, IsRequired = true)] public bool Bound;

        [ProtoMember(16, IsRequired = true)] public Enums.Color Color;

        [ProtoMember(17, IsRequired = true)] public int CraftProgress;

        [ProtoMember(15, IsRequired = true)] public int dwParam15;

        [ProtoMember(7, IsRequired = true)] public int dwParam7;

        [ProtoMember(6, IsRequired = true)] public Enums.ItemEffect Effect;

        [ProtoMember(12, IsRequired = true)] public bool Equipped;

        [ProtoMember(4, IsRequired = true)] public int FirstSocket;

        [ProtoMember(11, IsRequired = true)] public int Health;
        ...
        /// </summary>
        [ProtoMember(18, IsRequired = true)] public bool Inscribed;

        [ProtoMember(2, IsRequired = true)] public int ItemID;

        [ProtoMember(1, IsRequired = true)] public uint ItemUID;

        [ProtoMember(14, IsRequired = true)] public bool Locked;

        [ProtoMember(23, IsRequired = true)] public int MaximumDurability;

        [ProtoMember(22, IsRequired = true)] public int MinimumDurability;

        [ProtoMember(8, IsRequired = true)] public int Plus;

        [ProtoMember(9, IsRequired = true)] public int Protection;

        [ProtoMember(5, IsRequired = true)] public int SecondSocket;

        [ProtoMember(3, IsRequired = true)] public int SocketProgress;

        [ProtoMember(21, IsRequired = true)] public int Stack;

        /// <summary>
        /// Time left in minutes (if item not activated only)
        /// </summary>
        [ProtoMember(20, IsRequired = true)] public int SubTime;

        [ProtoMember(13, IsRequired = true)] public bool Suspicious;

        /// <summary>
        /// Time left in seconds !
        /// </summary>
        [ProtoMember(19, IsRequired = true)] public int Time;
    }
}