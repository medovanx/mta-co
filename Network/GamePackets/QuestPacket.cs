...
namespace MTA.Network.GamePackets {
    ...
    public class QuestPacket : Writer, IPacket {
    ...
    public class Quests {
                ...
                using (MySqlReader rdr = new MySqlReader(cmd)) {
                    if (rdr.Read()) {
                        ...
                        using (var command = new MySqlCommand(MySqlCommandType.INSERT)) {
                            command.Insert("quests").Insert("UID", Player.Entity.UID)
                                .Insert("Name", Player.Entity.Name);
                            command.Execute();
                        }
                    }
                }
        ...
        public QuestItem Accept(QuestID UID, uint time = 0) {
            if (!src.ContainsKey(UID)) {
                var n_quest =
                    QuestItem.Create(
                        QuestPacket.QuestData.Create(UID, QuestPacket.QuestData.QuestStatus.Accepted, time), 0);
        ...
        public void SendFullGUI() {
            ...
            Dictionary<int, Queue<QuestPacket.QuestData>> Collection =
                new Dictionary<int, Queue<QuestPacket.QuestData>>();
    ...
    public class QuestData : Writer, IPacket {
        byte[] Buffer;
