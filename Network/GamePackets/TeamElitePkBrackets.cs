...
namespace MTA.Network.GamePackets {
    public class TeamElitePkBrackets : Writer, IPacket {
        public const byte
            InitialList = 0,
            StaticUpdate = 1,
            GUIEdit = 2,
        ...
        private void AppendPlayer(TeamElitePk.FighterStats stats, int _offset) {
            WriteUint(stats.Team.UID, _offset, Buffer);
            _offset += 4;
            WriteUint(stats.LeaderUID, _offset, Buffer);
            _offset += 4;
            WriteUint(stats.LeaderMesh, _offset, Buffer);
            _offset += 4;
            WriteString(stats.Name, _offset, Buffer);
            _offset += 32;
            WriteUint((uint)stats.Flag, _offset, Buffer);
            _offset += 4;
        }
    }
}