    ...
    private void timerCallback(int tim)
    {
        try
        {
            ...
            if (State == States.GUI_ReconstructTop)
            {
                ...
                {
                    State = States.GUI_Top8Ranking;
                    Kernel.SendWorldMessage(new Network.GamePackets.TeamElitePkBrackets(ID)
            ...
            if (Now64 > StarTimer)
            {
                ...
                if (State == States.GUI_Top8Ranking)
                {
                    ...
                    Kernel.SendWorldMessage(new Network.GamePackets.TeamElitePkBrackets(ID) { Type = 6, OnGoing = true }
                        .ToArray());
                    foreach (var team in Teams.Values)
                    {
                        foreach (var player in team.Players)
                        {
                            if (player != null)
                            {
                                if (player.Team != null)
                                {
                                    Network.GamePackets.SkillEliteSetTeamName teamname =
                                        new Network.GamePackets.SkillEliteSetTeamName(ID);
                                    {
                        ...
                        if (GroupID == 3)
                        {
                            ...
                            for (int i = 0; i < count; i++)
                            {
                                var ai = new MaTrix.AI(WaitingArea.ID, 50,
                                    50, MaTrix.AI.BotLevel.MaTrix,
                                    (po) => { return Client.GameState.IsVaildForTeamPk(po); });
                                if (ai.Bot.Team == null)
                ...
                if (State == States.GUI_Top8Qualifier)
                {
                    if (pState == States.T_Organize)
                    {
                        ...
                        else
                        {
                            ...
                            var brackets = CreateBrackets(MatchArray, 0,
                                Network.GamePackets.TeamElitePkBrackets.UpdateList);
                            foreach (var packet in brackets)
                            {
                                Kernel.SendWorldMessage(packet.ToArray());
                            }

                            Network.GamePackets.TeamElitePkBrackets brea =
                                new Network.GamePackets.TeamElitePkBrackets(0);
    ...
    private Network.GamePackets.TeamElitePkBrackets[] CreateBrackets(Match[] matches, int page = 0, byte type = 0)
    {
        try
        {
            ...
            if (State == States.GUI_Knockout)
            {
                ...
                Network.GamePackets.TeamElitePkBrackets brackets =
                    new Network.GamePackets.TeamElitePkBrackets(ID, (int)ccount);
                brackets.Group = (ushort)GroupID;
            ...
            else if (State == States.GUI_Top8Qualifier)
            {
                ...
                if (matches.Length > ccount)
                {
                    ...
                    Network.GamePackets.TeamElitePkBrackets twobrackets =
                        new Network.GamePackets.TeamElitePkBrackets(ID, (int)towcount);
            ...
            if (State >= States.GUI_Top2Qualifier)
            {
                ...
                Network.GamePackets.TeamElitePkBrackets qbrackets =
                    new Network.GamePackets.TeamElitePkBrackets(ID, count);
                {
                ...
                if (State >= States.GUI_Top3)
                {
                    ...
                    Network.GamePackets.TeamElitePkBrackets Twoqbrackets =
                        new Network.GamePackets.TeamElitePkBrackets(ID, count);
                    {
