...
namespace MTA.Game.Items.Handlers {
    /// <summary>
    /// Handles SuperBowPack item that gives a Level 70 one Soc +5 Bow, small Praying Stone (B), EXP Ball (B), and 5 EXP Potions.
    /// </summary>
    [ItemHandler(SuperBowPack)]
    public static class SuperBowPackHandler {
            ...
            if (client.Entity.Level >= 10) {
                if (client.Inventory.Count < 32) {
                    ...
                    items.Durability = items.MaximDurability =
                        ConquerItemInformation.BaseInformations[GooseBow_Super].Durability;
