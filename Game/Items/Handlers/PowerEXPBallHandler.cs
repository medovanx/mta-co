...
namespace MTA.Game.Items.Handlers {
    /// <summary>
    /// Handles PowerEXPBall items that grant 20% of current level EXP when used (level smaller than 140).
    /// </summary>
    ...
    public static class PowerExpBallHandler {
        public static void Handle(GameState client, ConquerItem item) {
            if (client.Entity.Level < 140) {
                ...
                client.Send(new Message("You already reached the maximum level.", Color.Red, Message.TopLeft));
            }
        }
    }
}