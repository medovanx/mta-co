...
namespace MTA.Game.Items.Handlers {
    /// <summary>
    /// Handles SuperRingPack item that gives a Level 70 one Soc +5 Ring, small Praying Stone (B), EXP Ball (B), and 5 EXP Potions.
    /// </summary>
    [ItemHandler(SuperRingPack)]
    public static class SuperRingPackHandler {
            ...
            if (client.Entity.Level >= 10) {
                if (client.Inventory.Count < 32) {
                    ...
                    items.Durability = items.MaximDurability =
                        ConquerItemInformation.BaseInformations[PearlRing_Super].Durability;
