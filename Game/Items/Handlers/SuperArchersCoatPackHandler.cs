...
namespace MTA.Game.Items.Handlers {
    /// <summary>
    /// Handles SuperArcher`sCoatPack item that gives a Level 67 one Soc +5 Archer`s Coat, small Praying Stone (B), EXP Ball (B), 5 EXP Potions.
    /// </summary>
    [ItemHandler(SuperArchersCoatPack)]
    public static class SuperArchersCoatPackHandler {
            ...
            if (client.Entity.Level >= 10) {
                if (client.Inventory.Count < 32) {
                    ...
                    items.Durability = items.MaximDurability =
                        ConquerItemInformation.BaseInformations[ApeCoat_Super].Durability;
