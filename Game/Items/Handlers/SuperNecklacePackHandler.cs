...
namespace MTA.Game.Items.Handlers {
    /// <summary>
    /// Handles SuperNecklacePack item that gives a Level 67 one Soc +5 Necklace, small Praying Stone (B), EXP Ball (B), and 5 EXP Potions.
    /// </summary>
    [ItemHandler(SuperNecklacePack)]
    public static class SuperNecklacePackHandler {
            ...
            if (client.Entity.Level >= 10) {
                if (client.Inventory.Count < 32) {
                    ...
                    var items = new ConquerItem(true) {
                        ID = PlatinaNecklace_Super,
                        Color = Enums.Color.White,
                        Plus = 5,
                        SocketOne = Enums.Gem.EmptySocket
                    };
                    items.Durability = items.MaximDurability =
                        ConquerItemInformation.BaseInformations[PlatinaNecklace_Super].Durability;
