...
namespace MTA.Game.Items.Handlers.SkillBooks {
    ...
    public static class FastBladeHandler {
        public static void Handle(GameState client, ConquerItem item) {
            ...
            if (SpellTable.SpellInformations[Spells.FastBlade][0].WeaponSubtype
                .Where(prof => client.Proficiencies.ContainsKey(prof))
                .Any(prof => client.Proficiencies[prof].Level >= 5)) {
                client.Inventory.Remove(item, Enums.ItemUse.RemoveFromStack);
                client.AddSpell(new Spell(true) { ID = Spells.FastBlade });
                return;
            }

            client.MessageBox("You need level 5 at blade proficiency!");
        }
    }
}