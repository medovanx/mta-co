...
namespace MTA.Game.Items.Handlers {
    /// <summary>
    /// Handles SuperArcher`sHatPack item that gives a Level 72 one Soc +5 Archer`s Hat, small Praying Stone (B), EXP Ball (B), 5 EXP Potions.
    /// </summary>
    [ItemHandler(SuperArchersHatPack)]
    public static class SuperArchersHatPackHandler {
            ...
            if (client.Entity.Level >= 10) {
                if (client.Inventory.Count < 32) {
                    ...
                    items.Durability = items.MaximDurability =
                        ConquerItemInformation.BaseInformations[ApeHat_Super].Durability;
