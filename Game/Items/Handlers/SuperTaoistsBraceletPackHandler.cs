...
namespace MTA.Game.Items.Handlers {
    /// <summary>
    /// Handles SuperTaoist`sBraceletPack item that gives a Level 75 one Soc +5 Taoist`s Bracelet, small Praying Stone (B), EXP Ball (B), and 5 EXP Potions.
    /// </summary>
    [ItemHandler(SuperTaoistsBraceletPack)]
    public static class SuperTaoistsBraceletPackHandler {
            ...
            if (client.Entity.Level >= 10) {
                if (client.Inventory.Count < 32) {
                    ...
                    var items = new ConquerItem(true) {
                        ID = CyaniteBracelet_Super,
                        Color = Enums.Color.White,
                        Plus = 5,
                        SocketOne = Enums.Gem.EmptySocket
                    };
                    items.Durability = items.MaximDurability =
                        ConquerItemInformation.BaseInformations[CyaniteBracelet_Super].Durability;
