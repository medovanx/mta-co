...
namespace MTA.Game {
    ...
    class CrossServer {
        ...
        public static void End() {
            ...
            if (PoleKeeper != null) {
                Kernel.SendWorldMessage(
                    new Message("[CrossServer CTF], " + PoleKeeper.Name + ", has won this CrossServer CTF", Color.White,
        ...
        public static void Start() {
            ...
            Kernel.SendWorldMessage(new Message("CrossServer CTF Has began!", Color.Red, Message.Center),
                Program.Values);
            foreach (var client in Kernel.GamePool.Values) {
        ...
        private static byte[] generateList(uint m, int type, Countrys[] array = null,
            Func<Countrys, UInt32> select = null) {
            byte[] data = null;
            if (array == null)
                data = new byte[708];
            else
            ...
            if (array == null)
                return data;
            Writer.WriteInt32(700, 0, data);
            Writer.WriteUInt16(1063, 2, data);
            Writer.WriteInt32(type, 4, data);
            Writer.WriteInt32(3, 6, data);
            Writer.WriteInt32(array.Length, 10, data);
            Writer.WriteInt32(8, 14, data);
            if (array != null) {
                uint b = 0;
                uint num2 = (m - 1u) * 8u;
                int offset = 30;
                while (b < array.Length) {
                    if (b >= num2) {
                        ...
                        offset += 36;
                        Writer.WriteUInt32(array[(int)b].wins, offset, data);
                        offset += 4;
                        Writer.WriteUInt32(8, offset, data);
                        offset += 4;
                    }
