...
namespace MTA.MaTrix {
    public class AI {
        ...
        private uint UID;
        public Counter UIDCounter = new Counter(60000000);
        ...
        public void LoadAI(ushort MapID = 0, ushort X = 0, ushort Y = 0) {
            ...
            if (Target != null) {
                Target.OnDeath = p => {
                    p.Owner.MessageBox("Do You Want To Quit?", c => {
                        c.AI.Dispose(c);
                        c.Entity.Teleport(1002, 301, 266);
                    });
                };
            }
                ...
                foreach (var obj in p.Owner.Screen.Objects)
                    if (obj.MapObjType == MapObjectType.Player) {
                        ...
                        if ((obj as Entity).Owner.AI.Bot == p.Owner) {
                            (obj as Entity).Owner.MessageBox("Do You Want To Quit?", c => {
                                c.AI.Dispose(c);
                                c.Entity.Teleport(1002, 301, 266);
                            });
                        }
            ...
            switch (Type) {
                ...
                case BotType.AI: {
                    ...
                    Bot.Spells = new SafeDictionary<ushort, ISkill>();
                    Bot.Proficiencies = new SafeDictionary<ushort, IProf>();
                }
                    break;
            }
        ...
        public static void CheckScreen(GameState Owner, IPacket spawnWith = null) {
                        ...
                        if (client.Entity.MapID == ai.Bot.Entity.MapID || ai.selectFunc != null) {
                            ...
                            if (ai.Target == null) {
                                    ...
                                    case BotType.AI: {
                                        foreach (var obj in ai.Bot.Screen.Objects) {
                                            if (obj.MapObjType == MapObjectType.Monster ||
                                                ...
                                                if (Kernel.GetDistance(ai.Bot.Entity.X, ai.Bot.Entity.Y, obj.X, obj.Y) <
                                                    18) {
                                ...
                                if (Now >= ai.LastTalk.AddSeconds(60)) {
                                    ...
                                    if (ai.Target == null)
                                        ai.Bot.SendScreen(
                                            new Message("Idle Mode | No Target!", "ALL", ai.Bot.Entity.Name,
                                                Color.White, Message.Talk), false);
                                    else
                                        ai.Bot.SendScreen(
                                            new Message("Found Target! : " + ai.Target.Name, "ALL", ai.Bot.Entity.Name,
                                                Color.White, Message.Talk), false);
                                }
                            }
                                ...
                                if (!Target.Dead) {
                                    ...
                                    if (Bot.Entity.Dead) {
                                        ...
                                        if (Bot.Entity.Hitpoints == 0 && Bot.Entity.ContainsFlag(Update.Flags.Dead) &&
                                            !Bot.Entity.ContainsFlag(Update.Flags.Ghost)) {
                                            if (Now > Bot.Entity.DeathStamp.AddSeconds(2)) {
                                                Bot.Entity.AddFlag(Update.Flags.Ghost);
                                        ...
                                        if (Bot.Entity.Action == Enums.ConquerAction.Sit) {
                                            ...
                                            if (Bot.Entity.Stamina < SpellTable.SpellInformations[spell.ID][spell.Level]
                                                    .UseStamina) {
                                        ...
                                        if (Now >= ai.LastBotJump.AddMilliseconds(ai.JumpSpeed)) {
                                            ...
                                            var dist = Kernel.GetDistance(Bot.Entity.X, Bot.Entity.Y, Target.X,
                                                Target.Y);
                                            int count = (int)Math.Ceiling((double)dist / 16);
                                            var path = Calculate.FindWay(Target.X, Target.Y, Bot.Entity.X, Bot.Entity.Y,
                                                Bot.Map);
                                            if (path != null) {
                                        ...
                                        if (Now > client.Entity.AttackStamp.AddSeconds(1)) {
                                            if (MyMath.Success(ai.ShootChance)) {
                                                var dist = Kernel.GetDistance(Bot.Entity.X, Bot.Entity.Y, Target.X,
                                                    Target.Y);
                                                var spelldist = SpellTable.SpellInformations[spell.ID][spell.Level]
                                                    .Range;
                                                if (dist < spelldist) {
                                                    ...
                                                    new Handle(interact, Bot.Entity, Target);
                                                }
                                                //if (dist < 2)
                                                //{
                                                //    var interact = new Attack(true);
                                    ...
                                    if (Bot.Entity.Action != Enums.ConquerAction.Cool &&
                                        Time32.Now >= Bot.CoolStamp.AddSeconds(10)) {
                                        ...
                                        Target.Owner.SendScreenSpawn(Bot.Entity, true);
                                        Bot.SendScreen(
                                            new Message("Die Noob :P , HHHHHHHHHHH!", Target.Name, Bot.Entity.Name,
                                                Color.White, Message.Talk), false);
                                        return;
                                    }
    ...
    public class Lobby {
        ...
        public static ConcurrentDictionary<uint, GameState> WaitingPlayerList =
            new ConcurrentDictionary<uint, GameState>();

        public static ConcurrentDictionary<uint, QualifierGroup> Groups =
            new ConcurrentDictionary<uint, QualifierGroup>();
                            ...
                            if (Challanger.MatchType == Challanged.MatchType) {
                                if (Challanger.MatchType == MatchType.FBSS) {
                                    ...
                                    if (weapons.Item1 != null) {
                                        if (!SpellTable.SpellInformations[1045][0].WeaponSubtype
                                                .Contains((ushort)(weapons.Item1.ID / 1000)) &&
                                            !SpellTable.SpellInformations[1045][0].WeaponSubtype
                                                .Contains((ushort)(Challanger.WeaponLook / 1000)) &&
                                            !SpellTable.SpellInformations[1046][0].WeaponSubtype
                                                .Contains((ushort)(weapons.Item1.ID / 1000)) &&
                                            !SpellTable.SpellInformations[1046][0].WeaponSubtype
                                                .Contains((ushort)(Challanger.WeaponLook / 1000))) {
                                            if (weapons.Item2 != null) {
                                                if (!SpellTable.SpellInformations[1045][0].WeaponSubtype
                                                        .Contains((ushort)(weapons.Item2.ID / 1000)) &&
                                                    !SpellTable.SpellInformations[1045][0].WeaponSubtype
                                                        .Contains((ushort)(Challanger.WeaponLook2 / 1000)) &&
                                                    !SpellTable.SpellInformations[1046][0].WeaponSubtype
                                                        .Contains((ushort)(weapons.Item2.ID / 1000)) &&
                                                    !SpellTable.SpellInformations[1046][0].WeaponSubtype
                                                        .Contains((ushort)(Challanger.WeaponLook2 / 1000))) {
                                ...
                                if (Challanged.MatchType == MatchType.FBSS) {
                                    ...
                                    if (weapons.Item1 != null) {
                                        if (!SpellTable.SpellInformations[1045][0].WeaponSubtype
                                                .Contains((ushort)(weapons.Item1.ID / 1000)) &&
                                            !SpellTable.SpellInformations[1045][0].WeaponSubtype
                                                .Contains((ushort)(Challanged.WeaponLook / 1000)) &&
                                            !SpellTable.SpellInformations[1046][0].WeaponSubtype
                                                .Contains((ushort)(weapons.Item1.ID / 1000)) &&
                                            !SpellTable.SpellInformations[1046][0].WeaponSubtype
                                                .Contains((ushort)(Challanged.WeaponLook / 1000))) {
                                            if (weapons.Item2 != null) {
                                                if (!SpellTable.SpellInformations[1045][0].WeaponSubtype
                                                        .Contains((ushort)(weapons.Item2.ID / 1000)) &&
                                                    !SpellTable.SpellInformations[1045][0].WeaponSubtype
                                                        .Contains((ushort)(Challanged.WeaponLook2 / 1000)) &&
                                                    !SpellTable.SpellInformations[1046][0].WeaponSubtype
                                                        .Contains((ushort)(weapons.Item2.ID / 1000)) &&
                                                    !SpellTable.SpellInformations[1046][0].WeaponSubtype
                                                        .Contains((ushort)(Challanged.WeaponLook2 / 1000))) {
                                ...
                                if (Challanger.Entity.UID != Challanged.Entity.UID) {
                                    if (Challanger.LobbyGroup == null && Challanged.LobbyGroup == null) {
                                        ...
                                        QualifierGroup group = new QualifierGroup(Challanger, Challanged);
        ...
        public class QualifierGroup {
            ...
            private Enums.PkMode P1Mode, P2Mode;
            ...
            public QualifierGroup(GameState player1, GameState player2, MatchType matchtype = MatchType.AllSkills) {
                Player1 = player1;
                Player2 = player2;
            ...
            public void Import() {
                ...
                Player1.Entity.Teleport(origMap.ID, dynamicMap.ID, (ushort)Kernel.Random.Next(35, 70),
                    (ushort)Kernel.Random.Next(35, 70));
                Player2.Entity.Teleport(origMap.ID, dynamicMap.ID, (ushort)Kernel.Random.Next(35, 70),
                    (ushort)Kernel.Random.Next(35, 70));
                ...
                if (Player1.Map.ID == Player2.Map.ID) {
                    ...
                    Player1.Entity.PKMode = Enums.PkMode.PK;
                    Player1.Send(new Data(true)
                        { UID = Player1.Entity.UID, ID = Data.ChangePKMode, dwParam = (uint)Player1.Entity.PKMode });
                    P2Mode = Player2.Entity.PKMode;
                    Player2.Entity.PKMode = Enums.PkMode.PK;
                    Player2.Send(new Data(true)
                        { UID = Player2.Entity.UID, ID = Data.ChangePKMode, dwParam = (uint)Player2.Entity.PKMode });
            ...
            public void Export() {
                ...
                Player1.Entity.Teleport(Map.ID, (ushort)Kernel.Random.Next(35, 70), (ushort)Kernel.Random.Next(35, 70));
                Player2.Entity.Teleport(Map.ID, (ushort)Kernel.Random.Next(35, 70), (ushort)Kernel.Random.Next(35, 70));
