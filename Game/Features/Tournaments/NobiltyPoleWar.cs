...
namespace MTA.Game {
    public class NobiltyPoleWar {
            ...
            if (IsWar) {
                ...
                client.OnDisconnect = p => {
                    ...
                    client.Send(new Data(true)
                        { UID = client.Entity.UID, ID = Data.ChangePKMode, dwParam = (uint)client.Entity.PKMode });
                };
        ...
        public static void EndWar() {
            Kernel.SendWorldMessage(new Message("NobiltyPole war has ended.!", Color.Red, Message.Center),
                Program.Values);
            IsWar = false;
            Map.Npcs.Clear();
            ...
            for (int i = 0; i < PolesWinners.Length; i++) {
                ...
                if (c.Entity.UID == winner) {
                    ...
                    Kernel.SendWorldMessage(
                        new Message(
                            "Congratulations! " + c.Entity.Name + " The winner NobiltyPoleWar Prize [ " +
                            cps.ToString() + " ] cps + 50 KPT!", Color.White, Message.Center), Program.Values);
        ...
        public static void FinishRound(SobNpcSpawn Pole, int _Class) {
            ...
            if (PoleKeeper != null) {
                Kernel.SendWorldMessage(
                    new Message("" + PoleKeeper.Name + " has won this round [NobiltyPoleWar]!", Color.Red,
