...
namespace MTA.Game {
    public class ClanArena {
        ...
        public void import() {
            ...
            foreach (var Member in Clan1.Members.Values) {
                ...
                if (import != null) {
                    if (CanFight(import)) {
                        import.MessageBox(
                            "Your Clan Will Fight In Clan Arena For [" + ValueFight + "]CPS You Like To Join", p => {
                                ...
                                p.Entity.Teleport(p.Entity.CLanArenaBattle.dynamicMap.ID,
        ...
        public void CheakToEnd(GameState gameClient, bool dc = false) {
            ...
            ClanSurive1 = Clan1.Members.Values.Where(mem => mem.Client != null).Any(mem =>
                !mem.Client.Entity.Dead && mem.Client.Entity.MapID == dynamicMap.ID);

            ClanSurive2 = Clan2.Members.Values.Where(mem => mem.Client != null).Any(mem =>
                !mem.Client.Entity.Dead && mem.Client.Entity.MapID == dynamicMap.ID);
            if (!ClanSurive1)
                End(Clan2, Clan1);
            else if (!ClanSurive2)
