        ...
        public static void GetReward(GameState client, uint cps) {
            if (client.Entity.GuildRank == (ushort)MemberRank.GuildLeader) {
                if (client.Entity.GuildID == Winner) {
                    ...
                    Kernel.SendWorldMessage(
                        new Message(
                            "Congratulations! " + client.Entity.Name + " The winner GuildScoreWar Prize [" +
                            cps.ToString() + " ] cps + 100 KPT!", Color.White, Message.Center), Program.Values);
