...
namespace MTA.Game
{
    public class PoleMagice
    {
        ...
        public static void PoleMagiceIni()
        {
            var Map = Kernel.Maps[3995];
            Pole = (SobNpcSpawn)Map.Npcs[818];
            LeftGate = (SobNpcSpawn)Map.Npcs[516028];
            RightGate = (SobNpcSpawn)Map.Npcs[516024];
        }
        ...
        public static void Start()
        {
            ...
            name = new object[] { "Quest PoleMagice Has Started Go To Guild Controller At TwinCity (261,215)" };
            Kernel.SendWorldMessage(new Message(string.Concat(name), "ALLUSERS", "PoleMagice", System.Drawing.Color.Red, 2500), Program.Values);
            Kernel.SendWorldMessage(new Message("PoleMagice has began!", System.Drawing.Color.Red, Message.Center), Program.Values);
            FirstRound = true;
            ...
            Kernel.SendWorldMessage(upd, Program.Values, (ushort)3995);
            upd.Clear();
        ...
        public static void FinishRound()
        {
            ...
            if (PoleKeeper != null)
            {
                ...
                Kernel.SendWorldMessage(new Message("The guild, " + PoleKeeper.Name + ", owned by " + PoleKeeper.LeaderName + " has won this PoleMagice round!", System.Drawing.Color.Red, Message.Center), Program.Values);
            ...
            Pole.Hitpoints = Pole.MaxHitpoints;
            Kernel.SendWorldMessage(Pole, Program.Values, (ushort)3995);
            Reset();
        }
        ...
        public static void End()
        {
            if (PoleKeeper != null)
            {
                Kernel.SendWorldMessage(new Message("The guild, " + PoleKeeper.Name + ", owned by " + PoleKeeper.LeaderName + " has won this PoleMagice!---PoleMagice has ended!", System.Drawing.Color.White, Message.Center), Program.Values);
                //MTA.Database.EntityTable.Status2(); 
            }
            else
            {
                Kernel.SendWorldMessage(new Message("PoleMagice has ended and there was no winner!", System.Drawing.Color.Red, Message.Center), Program.Values);
                //MTA.Database.EntityTable.Status2(); 
            }
            IsWar = false;
            Claim = true;
        ...
        public static void SendScores()
        {
            ...
            for (int c = 0; c < scoreMessages.Length; c++)
            {
                ...
                Kernel.SendWorldMessage(msg, Program.Values, (ushort)3995);
        ...
        private static void SortScores(out Guild winner)
        {
            ...
            int Place = 0;
            foreach (Guild guild in Scores.Values.OrderByDescending((p) => p.MaScore))
            {
                if (Place == 0)
                    winner = guild;
                string str = "No  " + (Place + 1).ToString() + ": " + guild.Name + "(" + guild.MaScore + ")";
                ret.Add(str);
                Place++;
                if (Place == 4)
                    break;
