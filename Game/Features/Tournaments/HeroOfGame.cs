...
namespace MTA.Game.Features.Tournaments {
    public class HeroOfGame {
        ...
        public void SendMapMessaj(string packet) {
            var data = new Message(packet, Color.Yellow, Message.Center);
            ...
            if (!SendInvitation) {
                ...
                if (open) {
                    if (Secounds == 30)
                        ...
                        byte[] Messaje = new Message("Alive Players : [ " + CkeckUPAlive() + " ]", Color.Yellow,
                            Message.FirstRightCorner).ToArray();
                        ...
                        if (CkeckUPAlive() == 1) {
                            ...
                            if (client != null) {
                                ...
                                statue = new Statue(client.Entity.SpawnPacket, 105177, Enums.ConquerAction.Cool,
                                    (byte)Enums.ConquerAngle.SouthWest, 318, 143, true);
                                statue = new Statue(client.Entity.SpawnPacket, 105177, Enums.ConquerAction.Wave,
                                    (byte)Enums.ConquerAngle.SouthEast, 301, 371, true);

                                client.Entity.ConquerPoints += 4000000;
                                client.Entity.AddTopStatus(Update.Flags2.Top3Trojan, 2, DateTime.Now.AddMinutes(58));
                                Kernel.SendWorldMessage(
                                    new Message(
                                        "Congratulation ! " + client.Entity.Name +
                                        " Win The Hero OF Game, He Receice " + 4000000 + " CPS!", Color.White,
