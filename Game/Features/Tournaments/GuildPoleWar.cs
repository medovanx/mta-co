...
namespace MTA.MaTrix {
    public class GuildPoleWar {
        ...
        public static void EndWar() {
            Kernel.SendWorldMessage(new Message("GuildPole war has ended.!", Color.Red, Message.Center),
                Program.Values);
            IsWar = false;
            Map.Npcs.Clear();
            ...
            for (int i = 0; i < PolesWinners.Length; i++) {
                ...
                if (c.Entity.UID == winner) {
                    ...
                    Kernel.SendWorldMessage(
                        new Message(
                            "Congratulations! " + c.Entity.Name + " The winner GuildPoleWar Prize [" + cps.ToString() +
                            " ] cps + 100 KPT!", Color.White, Message.Center), Program.Values);
        ...
        public static void FinishRound(SobNpcSpawn Pole, int _Class) {
            ...
            if (PoleKeeper != null) {
                Kernel.SendWorldMessage(
                    new Message("" + PoleKeeper.Name + " has won this round [GuildPoleWar]!", Color.Red,
