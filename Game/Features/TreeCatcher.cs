...
namespace MTA.MaTrix.New_Quests {
    public class TreeCatcher {
        ...
        public void Create() {
            ...
            Console.WriteLine(string.Format("Tree Location : Map {0}, X {1}, Y{2}", ((Enums.Maps)Npc.MapID).ToString(),
                Npc.X, Npc.Y));
        }
        ...
        private void TryPathFinding(Tuple<ushort, ushort> cord) {
            new Thread(() => {
                Move = true;
                var path = Calculate.FindWay(cord.Item1, cord.Item2, Npc.X, Npc.Y, Map);
                if (path != null) {
        ...
        public static void ReSpwan() {
            ...
            Kernel.SendWorldMessage(
                new Message(
                    string.Format("Tree Respwan at Location : Map {0}, X {1}, Y{2}", ((Enums.Maps)Npc.MapID).ToString(),
                        Npc.X, Npc.Y), Color.Red, Message.TopLeft), Program.Values);
        }
    }
}