            ...
            foreach (var pet in pets) {
                if (pet != null) {
                    ...
                    if (distance >= 8) {
                        ...
                        Enums.ConquerAngle facing = Kernel.GetAngle(pet.Entity.X, pet.Entity.Y,
                            pet.Entity.Owner.Entity.X, pet.Entity.Owner.Entity.Y);
                                ...
                                if (monster.MonsterInfo.InSight == 0) {
                                    if (client.Entity.AttackPacket != null) {
                                        if (client.Entity.AttackPacket.AttackType == Attack.Magic) {
                                            if (client.Entity.AttackPacket.Decoded) {
                                                if (SpellTable.SpellInformations.ContainsKey(
                                                        (ushort)client.Entity.AttackPacket.Damage)) {
                                                    var info = SpellTable
                                                        .SpellInformations[(ushort)client.Entity.AttackPacket.Damage]
                                                        .Values.ToArray()[
                                                            client.Spells[(ushort)client.Entity.AttackPacket.Damage]
                                                                .Level];
                                                    if (info.CanKill) {
                                                        monster.MonsterInfo.InSight =
                                                            client.Entity.AttackPacket.Attacked;
                                                    }
                                                }
                                            }
                                        }
                                    ...
                                    if (monster.MonsterInfo.InSight is > 400000 and < 600000 ||
                                        ...
                                        if (client.Screen.TryGetValue(monster.MonsterInfo.InSight, out attacked)) {
                                            if (Now > monster.AttackStamp.AddMilliseconds(monster.MonsterInfo
                                                    .AttackSpeed)) {
                                                ...
                                                if (attacked.Dead) {
                                                    ...
                                                    new Handle(null, monster, attacked);
                                            }
                                        }
                                        else
                                            monster.MonsterInfo.InSight = 0;
                        ...
                        case PetType.Attacker: {
                            ...
                            foreach (var obj in client.Screen.Objects) {
                                ...
                                if (obj.MapObjType == MapObjectType.Monster) {
                                    ...
                                    {
                                        if (Now > pet.Entity.AttackStamp.AddMilliseconds(
                                                1000 - client.Entity.Agility)) {
                                            pet.Entity.AttackStamp = Now;
                                            if (!attacked.Dead)
                                                new Handle(null, pet.Entity, attacked);
                                        }
                                    }
                                }
                                    ...
                                    if (attacked.Dead)
                                        continue;
                                    if (Handle.CanAttack(client.Entity, attacked, null, true)) {
                                        //  if (Kernel.GetDistance(pet.Entity.X, pet.Entity.Y, attacked.X, attacked.Y) <= 15)
                                        {
                                            if (Now > pet.Entity.AttackStamp.AddMilliseconds(
                                                    1000 - client.Entity.Agility)) {
                                    ...
                                    if (Handle.CanAttack(client.Entity, attackedSobNpc, null)) {
                                        //   if (Kernel.GetDistance(pet.Entity.X, pet.Entity.Y, attackedSobNpc.X, attackedSobNpc.Y) <= 15)
                                        {
                                            if (Now > pet.Entity.AttackStamp.AddMilliseconds(
                                                ...
                                                uint damage = Calculate.Melee(client.Entity, attackedSobNpc,
                                                    ref attack);
                                                suse.Effect1 = attack.Effect1;
                                                Handle.ReceiveAttack(pet.Entity, attackedSobNpc, attack, damage, null);
                                                suse.Attacker = pet.Entity.UID;
                            ...
                            foreach (var obj in client.Screen.Objects) {
                                if (obj.MapObjType == MapObjectType.Monster) {
                                    ...
                                    if (attacked.MonsterInfo != null) {
                                        if (attacked.MonsterInfo.InSight == client.Entity.UID ||
                                            attacked.MonsterInfo.InSight == pet.Entity.UID) {
                                            if (Now > pet.Entity.AttackStamp.AddMilliseconds(
                                                    1000 - client.Entity.Agility)) {
                                                pet.Entity.AttackStamp = Now;
                                                if (!attacked.Dead)
                                    ...
                                    if (attacked.AttackPacket != null) {
                                        if (attacked.AttackPacket.Attacked == client.Entity.UID ||
                                            attacked.AttackPacket.Attacked == pet.Entity.UID) {
                                            if (Now > pet.Entity.AttackStamp.AddMilliseconds(
                                                    1000 - client.Entity.Agility)) {
                                                pet.Entity.AttackStamp = Now;
                                                if (!attacked.Dead)
