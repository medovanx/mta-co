...
namespace MTA.Game.Features {
    public class BreakingDown {
        ...
        public BreakingDown(DateTime EndTime, ushort QuestMapID, ushort QuestLevel, uint HeroUID,
        ...
        public static void Work(int time) {
            foreach (var players in BreakingDownQuests.Values.ToList()) {
                ...
                if (monsters.Count != 0) {
                    foreach (var player in Program.Values.Where(s => s.Map.ID == players.MapID).ToList()) {
                        ...
                        player.Send(new Message(
                            "You didn`t kill all the monesters in the map, You have been sent back to TwilCity. ",
                            Color.Red, Message.Whisper));
                    }
                ...
                foreach (var player in Program.Values.Where(s => s.Map.ID == players.MapID).ToList()) {
                    player.Send(new Message("You Finished This Stage, You are Now in stage " + players.Stage, Color.Red,
                        Message.Whisper));
                    player.Send(new Data(true)
                        { UID = player.Entity.UID, dwParam = (RoundTime * 60), ID = Data.CountDown });
                    player.Send(new Data(true)
                        { UID = player.Entity.UID, wParam1 = x, wParam2 = y, ID = Data.TeamSearchForMember });
                }
            }
        }
        ...
        public static void Add(GameState client, QuestTypes questTypes) {
            lock (SyncRoot) {
                ...
                if (BreakingDownQuests.ContainsKey(UID)) {
                    ...
                    client.Send(new Data(true)
                        { UID = client.Entity.UID, dwParam = (RoundTime * 60), ID = Data.CountDown });
                }
            }
        }
    }
