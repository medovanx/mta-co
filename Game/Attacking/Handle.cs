...
namespace MTA.Game.Attacking {
    public class Handle {
        ...
        public Entity findClosestTarget(Entity attacker, ushort X, ushort Y, IEnumerable<IMapObject> Array) {
        ...
        public static IEnumerable<GameState> PlayerinRange(Entity attacker, Entity attacked) {
            var dictionary = Kernel.GamePool.Values.ToArray();
            return dictionary.Where((player) =>
                player.Entity.MapID == attacked.MapID &&
                Kernel.GetDistance(player.Entity.X, player.Entity.Y, attacker.X, attacker.Y) <= 7);
        }
        ...
        public static IEnumerable<GameState> PlayerinRange(FloorItem item, int dist) {
            var dictionary = Kernel.GamePool.Values.ToArray();
            return dictionary
                .Where((player) => player.Entity.MapID == item.MapID &&
                                   Kernel.GetDistance(player.Entity.X, player.Entity.Y, item.X, item.Y) <= dist)
                .OrderBy(player => Kernel.GetDistance(player.Entity.X, player.Entity.Y, item.X, item.Y));
        }
        ...
        public static void FlameLotus(FloorItem item, int count = 0) {
            ...
            foreach (var target in targets) {
                uint damage = Calculate.Magic(client.Entity, target.Entity, ref attack);
                if (client.Spells.ContainsKey(1002)) {
                    var spell2 = SpellTable.GetSpell(client.Spells[1002].ID, client.Spells[1002].Level);
                    damage = Calculate.Magic(client.Entity, target.Entity, spell2, ref attack);
                }
            ...
            Program.World.DelayedTask.StartDelayedTask(() => {
                ...
                if (i != 0) {
                    ...
                    foreach (IMapObject _obj in attacker.Owner.Screen.Objects) {
                        ...
                        if (Kernel.GetDistance(_obj.X, _obj.Y, (ushort)algo.lcoords[selected].X,
                                (ushort)algo.lcoords[selected].Y) <= spell.Range)
                            hit = true;
                        if (hit) {
                            if (_obj.MapObjType == MapObjectType.Monster) {
                                attacked = _obj as Entity;
                                if (CanAttack(attacker, attacked, spell, attack.AttackType == Attack.Melee)) {
                                    var damage = Calculate.Melee(attacker, attacked, spell, ref attack);
            ...
            if (attack == null) {
                ...
                if (attacked.EntityFlag == EntityFlag.Player) {
                    ...
                    if (attacker.MonstersSpells != null) {
                            ...
                            default: {
                                {
                                    ...
                                    foreach (var attk in Inrage) {
                                        ...
                                        if (attackedd.ContainsFlag(Update.Flags.Frightened))
                                            continue;
                                        uint damage =
                                            Calculate.CalculateBossDamage(attacker, attacked,
                                                usespell); //Calculate.Magic(attacker, attk.Entity, usespell, 0, ref attack);
                        ...
                        (attacker.MonsterInfo.SpellID ==
                         0) // from this bracket to the next die delay replce this with urs
                    {
                        attack = new Attack(true);
                        attack.Effect1 = Attack.AttackEffects1.None;
                        attack.AttackType = Attack.Melee;
            ...
            #region Player -> Player \ Monster \ Sob Npc
                    ...
                    if (attack.AttackType == Attack.MarriageRequest) {
                        ...
                        if (Kernel.GamePool.TryGetValue(takeout, out Spouse)) {
                            if (attacker.Spouse != "None" || Spouse.Entity.Spouse != "None") {
                                attacker.Owner.Send(new Message(
                                    "You cannot marry someone that is already married with someone else!", Color.Black,
                                    Message.TopLeft));
                            }
                            else {
                                ...
                                if (id1 <= 2 && id2 >= 3 || id1 >= 2 && id2 <= 3) {
                                    ...
                                    attacker.Owner.Send(new Message("You cannot marry someone of your gender!",
                                        Color.Black, Message.TopLeft));
                                }
                            }
                        }
                    }
                        ...
                        if (Kernel.GamePool.TryGetValue(attack.Attacked, out Spouse)) {
                            if (attacker.Spouse != "None" || Spouse.Entity.Spouse != "None") {
                                ...
                                if (attacker.Mesh % 10 <= 2 && Spouse.Entity.Mesh % 10 >= 3 ||
                                    attacker.Mesh % 10 >= 3 && Spouse.Entity.Mesh % 10 <= 2) {
                                    Spouse.Entity.Spouse = attacker.Name;
                                    attacker.Spouse = Spouse.Entity.Name;

                                    // Explicitly send Spouse updates to refresh profile
                                    Spouse.Entity.Update(_String.Spouse, attacker.Name, true);
                                    attacker.Update(_String.Spouse, Spouse.Entity.Name, true);

                                    Message message = null;
                                    if (Spouse.Entity.Mesh % 10 >= 3)
                                        message = new Message(
                                            "Joy and happiness! " + Spouse.Entity.Name + " and " + attacker.Name +
                                            " have joined together in the holy marriage. We wish them a stone house.",
                                            Color.BurlyWood, Message.Center);
                                    else
                                        message = new Message(
                                            "Joy and happiness! " + attacker.Name + " and " + attacker.Spouse +
                                            " have joined together in the holy marriage. We wish them a stone house.",
                                            Color.BurlyWood, Message.Center);
                ...
                #region Attacking
                    ...
                    if (attacker.OnCyclone())
                        decrease =
                            1; //Franko Samak :  to fix the cyclone in scatter issue .. the value replaced from 700 to 1;
                    ...
                    if (attack.AttackType == Attack.Magic) {
                        if (!attack.Decoded) {
                            ...
                            Target = ((uint)attack.ToArray()[16] & 0xFF) | (((uint)attack.ToArray()[17] & 0xFF) << 8) |
                                     (((uint)attack.ToArray()[18] & 0xFF) << 16) |
                                     (((uint)attack.ToArray()[19] & 0xFF) << 24);
                            Target = ((((Target & 0xffffe000) >> 13) | ((Target & 0x1fff) << 19)) ^ 0x5F2D2463 ^
                                      attacker.UID) - 0x746F4AE6;
                    ...
                    if (!pass && attack.AttackType == Attack.Magic) {
                        if (!(doWep1Spell || doWep2Spell)) {
                            if (SpellID == 11960 || SpellID == 1045 || SpellID == 1046 || SpellID == 11005 ||
                                SpellID == 11000 || SpellID == 1100) // FB and SS
                            {
                                //do checks
                            }
                            else {
                    ...
                    if (SpellID == 1051) {
                        if (attacker.MapID == DeathMatch.MAPID) {
                            attacker.Owner.Send(new Message(
                                "You have to use manual linear skills(FastBlade/ScentSword)", Color.Red, Message.Talk));
                            return;
                        }
                        ...
                        foreach (IMapObject obj in attacker.Owner.Screen.Objects) {
                            if (obj == null)
                                continue;
                            if (obj.X == x && obj.Y == y && (obj.MapObjType == MapObjectType.Monster ||
                    ...
                    #region Melee
                        ...
                        if (attacker.Assassin() || attacker.IsBowEquipped) {
                            ...
                            new Handle(attack, attacker, attacked);
                            return;
                        }
                                ...
                                foreach (IMapObject _obj in attacker.Owner.Screen.Objects) {
                                    ...
                                    if (_obj.MapObjType == MapObjectType.Monster ||
                                        ...
                                        if (Kernel.GetDistance(attacker.X, attacker.Y, attacked.X, attacked.Y) <= 7) {
                                            if (CanAttack(attacker, attacked, null,
                                                    attack.AttackType == Attack.Melee)) {
                                                attack.Effect1 = Attack.AttackEffects1.None;
                                                uint damage = Calculate.Melee(attacker,
                                                    attacked, ref attack);
                                                attack.Attacked = 0;
                                                attack.Damage = damage;
                                                suse.Effect1 = attack.Effect1;
                                    ...
                                    if (_obj.MapObjType == MapObjectType.Monster ||
                                        ...
                                        if (Kernel.GetDistance(attacker.X, attacker.Y, attacked.X, attacked.Y) <= 7) {
                                            if (CanAttack(attacker, attacked, null,
                                                    attack.AttackType == Attack.Melee)) {
                                                attack.Effect1 = Attack.AttackEffects1.None;
                                                uint damagee = Calculate.Melee(attacker, attacked, ref attack);
                            ...
                            if (Kernel.GetDistance(attacker.X, attacker.Y, attacked.X, attacked.Y) <= range || pass) {
                                ...
                                if (attacker.OnFatalStrike()) {
                                    if (attacked.EntityFlag == EntityFlag.Monster) {
                                        ...
                                        ushort x = attacked.X;
                                        ushort y = attacked.Y;
                                        Map.UpdateCoordonatesForAngle(ref x, ref y,
                                            Kernel.GetAngle(attacked.X, attacked.Y, attacker.X, attacker.Y));
                                        attacker.Shift(x, y);
                                        attack.X = x;
                                        attack.Y = y;
                                        attack.Damage = damage;
                                ...
                                if (weapons.Item1 != null) {
                                    ...
                                    if (SpellTable.WeaponSpells.ContainsKey(wep1subyte)) {
                                        var weaponskill = SpellTable.WeaponSpells[wep1subyte];
                                        for (int i = 0; i < weaponskill.Count; i++) {
                                            if (!doWep1Spell) {
                                                wep1spellid = weaponskill[i];
                                                if (attacker.Owner.Spells.ContainsKey(wep1spellid) &&
                                                    SpellTable.SpellInformations.ContainsKey(wep1spellid)) {
                                                    wep1spell = SpellTable.SpellInformations[wep1spellid][
                                                        attacker.Owner.Spells[wep1spellid].Level];
                                    ...
                                    if (!doWep1Spell) {
                                        if (weapons.Item2 != null) {
                                            ...
                                            if (SpellTable.WeaponSpells.ContainsKey(wep2subyte)) {
                                                var weaponskill2 = SpellTable.WeaponSpells[wep2subyte];
                                                for (int i = 0; i < weaponskill2.Count; i++) {
                                                    if (!doWep2Spell) {
                                                        wep2spellid = weaponskill2[i];
                                                        if (attacker.Owner.Spells.ContainsKey(wep2spellid) &&
                                                            SpellTable.SpellInformations.ContainsKey(wep2spellid)) {
                                                            wep2spell = SpellTable.SpellInformations[wep2spellid][
                                                                attacker.Owner.Spells[wep2spellid].Level];
                                        ...
                                        if (attacker.EntityFlag == EntityFlag.Player &&
                                            ...
                                            if (damage > attacked.Hitpoints) {
                                                attacker.Owner.IncreaseProficiencyExperience(
                                                    Math.Min(damage, attacked.Hitpoints), wep1subyte);
                                                if (wep2subyte != 0) {
                                                    if (wep2bs)
                                                        wep2subyte++;
                                                    attacker.Owner.IncreaseProficiencyExperience(
                                                        Math.Min(damage, attacked.Hitpoints), wep2subyte);
                                                }
                                            }
                                            else {
                                    ...
                                    if (!attacker.Transformed) {
                                        if (attacker.EntityFlag == EntityFlag.Player &&
                                            ...
                                            if (damage > attacked.Hitpoints) {
                                                attacker.Owner.IncreaseProficiencyExperience(
                                                    Math.Min(damage, attacked.Hitpoints), 0);
                                            }
                                            else {
                            ...
                            if (CanAttack(attacker, attackedsob, null)) {
                                ...
                                if (Kernel.GetDistance(attacker.X, attacker.Y, attackedsob.X, attackedsob.Y) <= range) {
                                    ...
                                    if (weapons.Item1 != null) {
                                        ...
                                        if (SpellTable.WeaponSpells.ContainsKey(wep1subyte)) {
                                            var wep1 = SpellTable.WeaponSpells[wep1subyte];
                                        ...
                                        if (!doWep1Spell) {
                                            if (weapons.Item2 != null) {
                                                ...
                                                if (SpellTable.WeaponSpells.ContainsKey(wep2subyte)) {
                                                    var wep2 = SpellTable.WeaponSpells[wep2subyte];
                                            ...
                                            if (attacker.MapID == 1039) {
                                                ...
                                                if (attacker.EntityFlag == EntityFlag.Player)
                                                    if (damage > attackedsob.Hitpoints) {
                                                        attacker.Owner.IncreaseProficiencyExperience(
                                                            Math.Min(damage, attackedsob.Hitpoints), wep1subyte);
                                                        if (wep2subyte != 0) {
                                                            if (wep2bs)
                                                                wep2subyte++;
                                                            attacker.Owner.IncreaseProficiencyExperience(
                                                                Math.Min(damage, attackedsob.Hitpoints), wep2subyte);
                                                        }
                                                    }
                                                    else {
                                                        attacker.Owner.IncreaseProficiencyExperience(damage,
                                                            wep1subyte);
                                                        if (wep2subyte != 0) {
                                                            if (wep2bs)
                                                                wep2subyte++;
                    ...
                    #region Ranged
                        ...
                        if (attacker.Owner.Screen.TryGetValue(attack.Attacked, out attacked)) {
                            ...
                            if (attacker.ContainsFlag3(Update.Flags3.KineticSpark)) {
                                var spell = SpellTable.GetSpell(11590, attacker.Owner);
                                if (spell != null) {
                                    spell.CanKill = true;
                                    if (Kernel.Rate(spell.Percent)) {
                                        ...
                                        foreach (IMapObject _obj in attacker.Owner.Screen.Objects) {
                                            ...
                                            if (_obj.MapObjType == MapObjectType.Monster ||
                                                ...
                                                if (Kernel.GetDistance(lastAttacked.X, lastAttacked.Y, attacked1.X,
                                                        attacked1.Y) <= 5) {
                                                    if (CanAttack(attacker, attacked1, spell, false)) {
                                                        lastAttacked = attacked1;
                                                        uint damage = Calculate.Melee(attacker, attacked1, spell,
                                                            ref attack);
                                                        suse.Effect1 = attack.Effect1;
                                                ...
                                                if (Kernel.GetDistance(lastAttacked.X, lastAttacked.Y, attackedsob.X,
                                                        attackedsob.Y) <= 5) {
                                                    if (CanAttack(attacker, attackedsob, spell)) {
                                                        lastAttacked = attackedsob;
                                                        uint damage = Calculate.Melee(attacker, attackedsob,
                                                            ref attack);
                                                        suse.Effect1 = attack.Effect1;
                            ...
                            if (Kernel.GetDistance(attacker.X, attacker.Y, attacked.X, attacked.Y) <=
                                ...
                                if (!attacker.Assassin())
                                    ...
                                    damage = Calculate.Melee(attacker, attacked, ref attack) / 3;
                                ...
                                if (attacker.EntityFlag == EntityFlag.Player &&
                                    ...
                                    if (damage > attacked.Hitpoints) {
                                        attacker.Owner.IncreaseProficiencyExperience(
                                            Math.Min(damage, attacked.Hitpoints), (ushort)(weapons.Item1.ID / 1000));
                                    }
                                    else {
                                        attacker.Owner.IncreaseProficiencyExperience(damage,
                                            (ushort)(weapons.Item1.ID / 1000));
                                    }
                            ...
                            if (CanAttack(attacker, attackedsob, null)) {
                                                ...
                                                if (_obj.MapObjType == MapObjectType.Monster ||
                                                    ...
                                                    if (Kernel.GetDistance(lastAttacked.X, lastAttacked.Y, attacked1.X,
                                                        ...
                                                        if (CanAttack(attacker, attacked1, spell, false)) {
                                                            ...
                                                            //  damage += (damage * 36) / 100;
                                                            if (damage == 0) break;
                                                            ReceiveAttack(attacker, attacked1, attack, ref damage,
                                                                spell);
                                                            suse.AddTarget(attacked1, damage, attack);
                                                        }
                                                    }
                                                    ...
                                                    if (Kernel.GetDistance(lastAttacked.X, lastAttacked.Y,
                                                            attackedsob1.X, attackedsob1.Y) <= 5) {
                                                        if (CanAttack(attacker, attackedsob1, spell)) {
                                                            lastAttacked = attackedsob1;
                                                            uint damage = Calculate.Melee(attacker, attackedsob1,
                                                                ref attack);
                                                            suse.Effect1 = attack.Effect1;
                                                            ...
                                                            //damage += (damage * 36) / 100;
                                                            if (damage == 0) break;
                                                            ReceiveAttack(attacker, attackedsob1, attack, damage,
                                                                spell);
                                ...
                                if (Kernel.GetDistance(attacker.X, attacker.Y, attackedsob.X, attackedsob.Y) <=
                                    ...
                                    if (damage > attackedsob.Hitpoints) {
                                        attacker.Owner.IncreaseProficiencyExperience(
                                            Math.Min(damage, attackedsob.Hitpoints), (ushort)(weapons.Item1.ID / 1000));
                                    }
                                    else {
                                        attacker.Owner.IncreaseProficiencyExperience(damage,
                                            (ushort)(weapons.Item1.ID / 1000));
                        ...
                        //     return;
                    {
                        ...
                        if (attacker.MapID == DeathMatch.MAPID) {
                            if (SpellID != 1045 && SpellID != 1046 && spellID != 11005) {
                                attacker.Owner.Send(new Message("You should use skills(FastBlade/ScentSword/ViperFang)",
                        ...
                        if (shuriken)
                            ...
                            if (SpellTable.SpellInformations[SpellID] != null &&
                                !SpellTable.SpellInformations[SpellID].ContainsKey(choselevel))
                        ...
                        attacked = null;
                        attackedsob = null;
                        if (attacker.Owner.Screen.TryGetValue(Target, out attacked) ||
                            attacker.Owner.Screen.TryGetSob(Target, out attackedsob) || Target == attacker.UID ||
                            spell.Sort != 1) {
                            if (Target == attacker.UID)
                                attacked = attacker;
                            if (attacked != null) {
                                if (attacked.Dead && spell.Sort != SpellSort.Revive && spell.ID != 10405 &&
                                    spell.ID != 10425) {
                                    attacker.AttackPacket = null;
                                    return;
                                }
                                    ...
                                    if (weapons.Item1 != null) {
                                        ...
                                        if (weapons.Item2 != null) {
                                            ...
                                            if (!spell.OnlyWithThisWeaponSubtype.Contains((ushort)firstwepsubtype) &&
                                                !spell.OnlyWithThisWeaponSubtype.Contains(
                                                    (ushort)(attacker.Owner.WeaponLook / 1000))) {
                                                if (!spell.OnlyWithThisWeaponSubtype
                                                        .Contains((ushort)secondwepsubtype) &&
                                                    !spell.OnlyWithThisWeaponSubtype.Contains(
                                                        (ushort)(attacker.Owner.WeaponLook2 / 1000))) {
                                                    attacker.AttackPacket = null;
                                                    return;
                                                }
                                            }
                            ...
                            if (!attacker.Owner.Fake) {
                                ...
                                if (attacker.MapID == DeathMatch2.MAPID ||
                                    ...
                                    if (SpellID != 1045 && SpellID != 1046 && spellID != 11005 && spellID != 11960) {
                                        attacker.Owner.Send(new Message(
                                            "You Should Use Skill's(FastBlade/ScentSword/ViperFang) Only", Color.Red,
                                            Message.Talk));
                                        return;
                                    }
                                }
                            ...
                            switch (spellID) {
                                        ...
                                        if (attacked != null) {
                                            if (Kernel.GetDistance(attacker.X, attacker.Y, attacked.X, attacked.Y) <=
                                                ...
                                                if (CanAttack(attacker, attacked, spell,
                                                    ...
                                                    if (spell.ID == 11030) {
                                                        attack.Effect1 = Attack.AttackEffects1.None;
                                                        damage = Calculate.Melee(attacker, attacked, spell, ref attack);
                                                    }
                                            ...
                                            if (Kernel.GetDistance(attacker.X, attacker.Y, attackedsob.X,
                                                    attackedsob.Y) <= spell.Distance) {
                                                SpellUse suse = new SpellUse(true);
                                                suse.Attacker = attacker.UID;
                                                suse.SpellID = spell.ID;
                                                suse.SpellLevel = spell.Level;
                                                ...
                                                if (CanAttack(attacker, attackedsob, spell)) {
                                                    PrepareSpell(spell, attacker.Owner);
                                                    attack.Effect1 = Attack.AttackEffects1.None;
                                                    uint damage = Calculate.Magic(attacker, attackedsob, spell,
                                                        ref attack);
                                                    suse.Effect1 = attack.Effect1;
                                ...
                                case 1195: {
                                    if (CanUseSpell(spell, attacker.Owner)) {
                                        ...
                                        if (spell.ID == 1190) {
                                            Experience = damage = Math.Min(damage,
                                                attacker.MaxHitpoints - attacker.Hitpoints);
                                            attacker.Hitpoints += damage;
                                        }
                                        else {
                                            Experience = damage = Math.Min(damage,
                                                (uint)(attacker.MaxMana - attacker.Mana));
                                        ...
                                        if (attackedsob != null) {
                                            ...
                                            if (Kernel.GetDistance(attacker.X, attacker.Y, attackedsob.X,
                                                ...
                                                damage = Math.Min(damage,
                                                    attackedsob.MaxHitpoints - attackedsob.Hitpoints);
                                            ...
                                            if (spell.Multi) {
                                                if (attacker.Owner.Team != null) {
                                                    PrepareSpell(spell, attacker.Owner);
                                                    foreach (GameState teammate in attacker.Owner.Team.Teammates) {
                                                        if (Kernel.GetDistance(attacker.X, attacker.Y,
                                                                teammate.Entity.X, teammate.Entity.Y) <=
                                                            spell.Distance) {
                                                            uint damage = spell.Power;
                                                            damage = Math.Min(damage,
                                                                teammate.Entity.MaxHitpoints -
                                                                teammate.Entity.Hitpoints);
                                                    ...
                                                    if (Kernel.GetDistance(attacker.X, attacker.Y, attacked.X,
                                                            attacked.Y) <= spell.Distance) {
                                                        PrepareSpell(spell, attacker.Owner);

                                                        uint damage = spell.Power;
                                                        damage = Math.Min(damage,
                                                            attacked.MaxHitpoints - attacked.Hitpoints);
                                                        attacked.Hitpoints += damage;
                                                        Experience += damage;
                                                        suse.AddTarget(attacked, damage, attack);
                                ...
                                case 1100: {
                                    if (attackedsob != null)
                                        return;
                                    if (attacked.MapID == 1234 || attacked.MapID == 1235 || attacked.MapID == 1236 ||
                                        attacked.MapID == 1237 || attacked.MapID == 1238 || attacked.MapID == 2014)
                                        return;
                                    ...
                                    if (Kernel.GetDistance(attacker.X, attacker.Y, attacked.X, attacked.Y) <=
                                        ...
                                        if (CanUseSpell(spell, attacker.Owner)) {
                                            ...
                                            attacked.Owner.Entity.Action = Enums.ConquerAction.None;
                                            ...
                                            if (Kernel.GetDistance(attacker.X, attacker.Y, X, Y) <= spell.Range) {
                                                if (spellID == 10315) {
                                                    foreach (IMapObject objs in objects.ToArray()) {
                                                        ...
                                                        if (objs.MapObjType == MapObjectType.Monster ||
                                                            objs.MapObjType == MapObjectType.Player) {
                                                            attacked = objs as Entity;
                                                            if (Kernel.GetDistance(attacker.X, attacker.Y, attacked.X,
                                                                    attacked.Y) <= spell.Range) {
                                                                if (CanAttack(attacker, attacked, spell,
                                                                        attack.AttackType == Attack.Melee)) {
                                                                    attack.Effect1 = Attack.AttackEffects1.None;
                                                                    uint damage = Calculate.Melee(attacker, attacked,
                                                                        ref attack, client_Spell.LevelHu2);

                                                                    suse.Effect1 = attack.Effect1;
                                                                    if (spell.Power > 0) {
                                                                        attack.Effect1 = Attack.AttackEffects1.None;
                                                                        damage = Calculate.Magic(attacker, attacked,
                                                                            spell, ref attack, client_Spell.LevelHu2);
                                                                        suse.Effect1 = attack.Effect1;
                                                                    }

                                                                    if (spell.ID == 8030) {
                                                                        attack.Effect1 = Attack.AttackEffects1.None;
                                                                        damage = Calculate.Ranged(attacker, attacked,
                                                                            ref attack, client_Spell.LevelHu2);
                                                                    }
                                                            ...
                                                            if (Kernel.GetDistance(attacker.X, attacker.Y,
                                                                    attackedsob.X, attackedsob.Y) <= spell.Range) {
                                                                if (CanAttack(attacker, attackedsob, spell)) {
                                                                    attack.Effect1 = Attack.AttackEffects1.None;
                                                                    uint damage = Calculate.Melee(attacker, attackedsob,
                                                                        ref attack);
                                                                    if (spell.Power > 0) {
                                                                        damage = Calculate.Magic(attacker, attackedsob,
                                                                            spell, ref attack);
                                                                    }

                                                                    attack.Effect1 = Attack.AttackEffects1.None;
                                                                    if (spell.ID == 8030)
                                                                        damage = Calculate.Ranged(attacker, attackedsob,
                                                                            ref attack);
                                                                    suse.Effect1 = attack.Effect1;
                                                                    ReceiveAttack(attacker, attackedsob, attack, damage,
                                                                        spell);
                                                        ...
                                                        if (_obj.MapObjType == MapObjectType.Monster ||
                                                            ...
                                                            if (Kernel.GetDistance(attacker.X, attacker.Y, attacked.X,
                                                                ...
                                                                if (CanAttack(attacker, attacked, spell,
                                                                        attack.AttackType == Attack.Melee)) {
                                                                    attack.Effect1 = Attack.AttackEffects1.None;
                                                                    uint damage = Calculate.Magic(attacker, attacked,
                                                                        spell, ref attack, client_Spell.LevelHu2);

                                                                    if (spell.ID == 1115) {
                                                                        attack.Effect1 = Attack.AttackEffects1.None;
                                                                        damage = Calculate.Melee(attacker, attacked,
                                                                            spell, ref attack, client_Spell.LevelHu2);
                                                                        damage = (uint)(damage * 0.5);
                                                                    }
                                                    ...
                                                    if (spell.ID == 10405 && attacked.Dead) {
                                                        ...
                                                        if (!attacked.ContainsFlag2(Update.Flags2.SoulShackle)) {
                                                            ...
                                                            if (attacked.Dead) {
                                                                ...
                                                                attacked.AddFlag(Update.Flags
                                                                    .Dead); //Flag them as dead... should not be needed. This is no movement
                                                                attacked.AddFlag2(Update.Flags2
                                                                    .SoulShackle); //Give them shackeld effect                                                         
                                                                attacked.ShackleStamp =
                                                                    Time32
                                                                        .Now; //Set stamp so source can remove the flag after X seconds
                                                                attacked.ShackleTime =
                                                                    90; //(short)(30 + 15 * spell.Level);//double checking here. Could be db has this wrong.
                                                                Update upgrade = new Update(true);
                                                                upgrade.UID = attacked.UID;
                                                                upgrade.Append(Update.SoulShackle
                                                                    , 111
                                                                    , 90, 0, spell.Level);
                                            ...
                                            if (Kernel.GetDistance(attacker.X, attacker.Y, attacked.X, attacked.Y) <=
                                                ...
                                                if (CanAttack(attacker, attacked, spell,
                                                    ...
                                                    if (attacked.Owner != null) {
                                                        attacker.Owner.UpdateQualifier(attacker.Owner, attacked.Owner,
                                                            attacked.Hitpoints);
                                                    }
                                        ...
                                        if (attacker.Owner.Team != null) {
                                            foreach (GameState teammate in attacker.Owner.Team.Teammates) {
                                                if (teammate.Entity.UID != attacker.UID) {
                                                    if (Kernel.GetDistance(attacker.X, attacker.Y, teammate.Entity.X,
                                                            teammate.Entity.Y) <= 18) {
                                                        teammate.XPCount += 20;
                                    ...
                                    //case 10490:
                                {
                                    if (CanUseSpell(spell, attacker.Owner)) {
                                        ...
                                        if (Kernel.GetDistance(attacker.X, attacker.Y, X, Y) <=
                                                ...
                                                if (_obj.MapObjType == MapObjectType.Monster ||
                                                    ...
                                                    if (Kernel.GetDistance(attacker.X, attacker.Y, attacked.X,
                                                        ...
                                                        if (CanAttack(attacker, attacked, spell,
                                                            ...
                                                            attack.Effect1 = Attack.AttackEffects1.None;
                                                            uint damage = Calculate.Melee(attacker, attacked, spell,
                                                                ref attack, client_Spell.LevelHu2);
                                                            suse.Effect1 = attack.Effect1;
                                        ...
                                        if (Kernel.GetDistance(attacker.X, attacker.Y, X, Y) <=
                                            ...
                                            if (attackedsob != null) {
                                                ...
                                                if (CanAttack(attacker, attacked, spell,
                                                    ...
                                                    if (spell.ID == 11140) {
                                                        attack.Effect1 = Attack.AttackEffects1.None;
                                                        damage = Calculate.Melee(attacker, attacked, ref attack,
                                                            client_Spell.LevelHu2);
                                                    }
                                                ...
                                                if (_obj.MapObjType == MapObjectType.Player ||
                                                    ...
                                                    if (attacked.MapObjType == MapObjectType.Monster)
                                                        if (attacked.MonsterInfo.Boss)
                                                            continue;
                                                    if (Kernel.GetDistance(X, Y, attacked.X, attacked.Y) <=
                                                        spell.Range) {
                                                        if (CanAttack(attacker, attacked, spell,
                                                                attack.AttackType == Attack.Melee)) {
                                                            int potDifference = attacker.BattlePower -
                                                                attacked.BattlePower;
                                            ...
                                            if (Kernel.GetDistance(attacker.X, attacker.Y, Xx, Yx) <= spell.Range) {
                                                ...
                                                if (attackedsob == null) {
                                                    if (CanAttack(attacker, attacked, spell,
                                                        ...
                                                        if (attacker.Owner.Spells.ContainsKey(11230) &&
                                                            !attacked.Dead) {
                                                            ...
                                                            if (spellz != null) {
                                                                if (Kernel.Rate(spellz.Percent)) {
                                                                    ...
                                                                    damage = Calculate.Melee(attacker, attacked,
                                                                        ref attack) / 2;
                                                                    ssuse.AddTarget(attacked,
                                                                        new SpellUse.DamageClass().Damage = damage,
                                                                        attack);
                                                                    ReceiveAttack(attacker, attacked, attack,
                                                                        ref damage, spellz);
                                                                    attacker.Owner.SendScreen(ssuse);
                                ...
                                case 11180: {
                                    if (attacked != null) {
                                        ...
                                        if (Kernel.GetDistance(attacker.X, attacker.Y, attacked.X, attacked.Y) <=
                                            spell.Distance) {
                                            if (CanUseSpell(spell, attacker.Owner)) {
                                                if (!Kernel.Rate(Math.Max(5,
                                                        100 - (attacked.BattlePower - attacker.BattlePower) / 5)))
                                                    return;
                                                PrepareSpell(spell, attacker.Owner);
                                                SpellUse suse = new SpellUse(true);
                                                suse.Attacker = attacker.UID;
                                                ...
                                                if (attacker.Owner.Map.Floor[newx, newy, MapObjectType.Player,
                                                        attacked]) {
                                                    suse.X = attacked.X = newx;
                                                    suse.Y = attacked.Y = newy;
                                                }

                                                SpellUse.DamageClass tar = new SpellUse.DamageClass();
                                                if (CanAttack(attacker, attacked, spell, false)) {
                                ...
                                case 11170: {
                                    if (CanUseSpell(spell, attacker.Owner)) {
                                        ...
                                        foreach (var t in attacker.Owner.Screen.Objects) {
                                            if (t == null)
                                                continue;
                                            if (t.MapObjType == MapObjectType.Player ||
                                                ...
                                                if (Kernel.GetDistance(X, Y, target.X, target.Y) <= spell.Range) {
                                                    if (CanAttack(attacker, target, spell, false)) {
                                                        tar.Damage = Calculate.Melee(attacker, target, spell,
                                                            ref attack, client_Spell.LevelHu2) / 2;
                                                        tar.Hit = true;
                                ...
                                case 7002: {
                                    ...
                                    if (attacked.ContainsFlag(Update.Flags.Ride) &&
                                        ...
                                        if ((attackedSteed = attacked.Owner.Equipment.TryGetItem(ConquerItem.Steed)) !=
                                            null) {
                                            if ((attackerSteed =
                                                    attacker.Owner.Equipment.TryGetItem(ConquerItem.Steed)) != null) {
                                                SpellUse suse = new SpellUse(true);
                                                suse.Attacker = attacker.UID;
                                                suse.SpellID = spell.ID;
                                                suse.SpellLevel = spell.Level;
                                                ...
                                                else if (attackedSteed.Plus == attackerSteed.Plus &&
                                                         attackedSteed.PlusProgress <= attackerSteed.PlusProgress)
                                                    attacked.RemoveFlag(Update.Flags.Ride);
                                                else
                                                    suse.Targets[attacked.UID].Hit = false;
                                                attacker.Owner.SendScreen(suse);
                                ...
                                case 7003: {
                                    ...
                                    foreach (IMapObject _obj in attacker.Owner.Screen.Objects) {
                                        ...
                                        if (_obj.MapObjType == MapObjectType.Player && _obj.UID != attacker.UID) {
                                            attacked = _obj as Entity;
                                            if ((attackedSteed =
                                                    attacked.Owner.Equipment.TryGetItem(ConquerItem.Steed)) != null) {
                                                if ((attackerSteed =
                                                        attacker.Owner.Equipment.TryGetItem(ConquerItem.Steed)) !=
                                                    null) {
                                                    if (Kernel.GetDistance(attacker.X, attacker.Y, attacked.X,
                                                            attacked.Y) <= attackedSteed.Plus) {
                                                        if (CanAttack(attacker, attacked, spell,
                                                                attack.AttackType == Attack.Melee)) {
                                                            suse.AddTarget(attacked, 0, attack);
                                                            if (attackedSteed.Plus < attackerSteed.Plus)
                                                                attacked.RemoveFlag(Update.Flags.Ride);
                                                                     ...
                                                                     attackedSteed.PlusProgress <=
                                                                     attackerSteed.PlusProgress)
                                                                attacked.RemoveFlag(Update.Flags.Ride);
                                                            else
                                                                suse.Targets[attacked.UID].Hit = false;
                                                        }
                                            ...
                                            if (_obj.MapObjType == MapObjectType.Monster ||
                                                ...
                                                if (sector.Inside(attacked.X, attacked.Y)) {
                                                    if (CanAttack(attacker, attacked, spell,
                                                            attack.AttackType == Attack.Melee)) {
                                                        attack.Effect1 = Attack.AttackEffects1.None;
                                                        uint damage = Calculate.Magic(attacker, attacked, spell,
                                                            ref attack, client_Spell.LevelHu2);
                                                        suse.Effect1 = attack.Effect1;
                                            ...
                                            if (_obj.MapObjType == MapObjectType.Monster ||
                                                ...
                                                if (sector.Inside(attacked.X, attacked.Y)) {
                                                    if (CanAttack(attacker, attacked, spell,
                                                            attack.AttackType == Attack.Melee)) {
                                                        attack.Effect1 = Attack.AttackEffects1.None;
                                                        uint damage = Calculate.Ranged(attacker, attacked, spell,
                                                            ref attack, client_Spell.LevelHu2);
                                                ...
                                                if (sector.Inside(attackedsob.X, attackedsob.Y)) {
                                                    if (CanAttack(attacker, attackedsob, spell)) {
                                                        ...
                                                        if (damage == 0)
                                                            damage = 1;
                                                        damage = Calculate.Percent((int)damage, spell.PowerPercent);
                                    ...
                                    if (CanUseSpell(spell, attacker.Owner)) {
                                        ...
                                        if (attacked != null) {
                                            if (CanAttack(attacker, attacked, spell,
                                                ...
                                                byte dist = 5;
                                                var angle = Kernel.GetAngle(attacker.X, attacker.Y, attacked.X,
                                                    attacked.Y);
                                                while (dist != 0) {
                                ...
                                case 10430: {
                                    if (CanUseSpell(spell, attacker.Owner)) {
                                        ...
                                        if (attacker.Owner.Team != null) {
                                            ...
                                            foreach (GameState teammate in attacker.Owner.Team.Teammates) {
                                                if (Kernel.GetDistance(attacker.X, attacker.Y, teammate.Entity.X,
                                                        teammate.Entity.Y) <= spell.Distance) {
                                            ...
                                            if (Kernel.GetDistance(attacker.X, attacker.Y, X, Y) <= 3) {
                                                for (int c = 0; c < attacker.Owner.Screen.Objects.Length; c++) {
                                                    ...
                                                    if (c >= attacker.Owner.Screen.Objects.Length)
                                                        break;
                                                    IMapObject _obj = attacker.Owner.Screen.Objects[c];
                                                    if (_obj == null)
                                                        continue;
                                                    if (_obj.MapObjType == MapObjectType.Monster ||
                                                        _obj.MapObjType == MapObjectType.Player) {
                                    ...
                                    if (CanUseSpell(spell, attacker.Owner)) {
                                        ...
                                        foreach (var t in attacker.Owner.Screen.Objects) {
                                            ...
                                            if (t.MapObjType == MapObjectType.Player ||
                                                ...
                                                if (Kernel.GetDistance(X, Y, target.X, target.Y) <= spell.Range) {
                                                    if (CanAttack(attacker, target, spell, false)) {
                                                        tar.Damage = Calculate.Ranged(attacker, target, spell,
                                                            ref attack);
                                                        tar.Hit = true;
                                                        tar.newX = target.X;
                                                        tar.newY = target.Y;
                                                        ...
                                                        if (map != null) {
                                                            if (map.Floor[tar.newX, tar.newY, MapObjectType.Player,
                                                                    attacker]) {
                                                                target.X = tar.newX;
                                                                target.Y = tar.newY;
                                                            }
                                                            ...
                                                            if (attacker.Owner.Map.Floor[tar.newX, tar.newY,
                                                                    MapObjectType.Player, attacker]) {
                                                                target.X = tar.newX;
                                                                target.Y = tar.newY;
                                                            }
                                                            else {
                                ...
                                case 11110: {
                                    if (CanUseSpell(spell, attacker.Owner)) {
                                        ...
                                        for (i = 0; i < algo.lcoords.Count; i++) {
                                            if (attacker.Owner.Map.Floor[algo.lcoords[i].X, algo.lcoords[i].Y,
                                        ...
                                        foreach (IMapObject _obj in Array) {
                                            bool hit = false;
                                            for (int j = 0; j < i; j++)
                                                if (Kernel.GetDDistance(_obj.X, _obj.Y, (ushort)algo.lcoords[j].X,
                                ...
                                case 11100: {
                                    if (CanUseSpell(spell, attacker.Owner)) {
                                        ...
                                        foreach (var t in
                                                 attacker.Owner.Screen
                                                     .Objects /*.OrderBy(x => Kernel.GetDistance(x.X, x.Y, attacker.Owner.Entity.X, attacker.Owner.Entity.Y))*/
                                                ) {
                                            if (t == null)
                                                continue;
                                            if (t.MapObjType == MapObjectType.Player ||
                                ...
                                case 11050: {
                                    if (attacker.Owner.Entity.ContainsFlag(Update.Flags.XPList)) {
                                        attacker.Owner.Entity.RemoveFlag(Update.Flags.XPList);
                                        attacker.Owner.Entity.AddFlag2(Update.Flags2.CannonBarrage);
                                    ...
                                    foreach (var t in attacker.Owner.Screen.Objects) {
                                        ...
                                        if (t.MapObjType == MapObjectType.Player ||
                                            t.MapObjType == MapObjectType.Monster) {
                                            var target = t as Entity;
                                            if (Kernel.GetDistance(attacker.Owner.Entity.X, attacker.Owner.Entity.Y,
                                                    target.X, target.Y) <= spell.Range) {
                                                if (CanAttack(attacker, target, spell, false)) {
                                                    tar.Damage = Calculate.Ranged(attacker, target, ref attack,
                                                        client_Spell.LevelHu2);
                                                    suse.AddTarget(target, tar, attack);
                                ...
                                case 11060: {
                                    if (attacker.Owner.Entity.ContainsFlag(Update.Flags.XPList)) {
                                        attacker.Owner.Entity.RemoveFlag(Update.Flags.XPList);
                                        attacker.Owner.Entity.AddFlag2(Update.Flags2.BlackbeardsRage);
                                    ...
                                    if (CanUseSpell(spell, attacker.Owner)) {
                                        ...
                                        if (attacker.Owner.Team != null) {
                                            foreach (var mate in attacker.Owner.Team.Teammates) {
                                                if (Kernel.GetDistance(attacker.X, attacker.Y, mate.Entity.X,
                                                        mate.Entity.Y) <= 4) {
                                                    ...
                                                    attacker.RemoveFlag3(Update.Flags3.MagicDefender);
                                                    mate.Entity.AddFlag3(Update.Flags3.MagicDefender);
                                                    mate.Entity.Update(mate.Entity.StatusFlag, mate.Entity.StatusFlag2,
                                                        mate.Entity.StatusFlag3, Update.MagicDefenderIcone, 0x80,
                                                        mate.Entity.MagicDefenderSecs, 0, false);
                                            ...
                                            attacker.Owner.Entity.Update(attacker.Owner.Entity.StatusFlag,
                                                attacker.Owner.Entity.StatusFlag2, attacker.Owner.Entity.StatusFlag3,
                                                Update.MagicDefenderIcone, 0x80,
                                                attacker.Owner.Entity.MagicDefenderSecs, 0, false);
                                    ...
                                    if (Time32.Now >= attacker.DefensiveStanceStamp.AddSeconds(10)) {
                                        if (CanUseSpell(spell, attacker.Owner)) {
                                            ...
                                            if (attacker.IsDefensiveStance) {
                                                ...
                                                attacker.AddFlag2(Update.Flags2.Fatigue);
                                                attacker.Update(attacker.Owner.Entity.StatusFlag,
                                                    attacker.Owner.Entity.StatusFlag2,
                                                    attacker.Owner.Entity.StatusFlag3, Update.DefensiveStance, 0x7E,
                                                    (uint)spell.Duration, (uint)(spell.Level + 1), false);
                                        ...
                                        attacker.Owner.Send(new Message(
                                            "You need to wait 10 seconds in order to cast the spell again!", Color.Red,
                                            Message.TopLeft));
                                    }
                                ...
                                case 11620: {
                                    ...
                                    if ((weps.Item1 != null && weps.Item1.ID / 1000 != 613) ||
                                        (weps.Item2 != null && weps.Item2.ID / 1000 != 613)) {
                                        attacker.Owner.Send(new Message("You need to wear only knifes!", Color.Red,
                                            Message.Talk));
                                        return;
                                    }
                                ...
                                case 11660: {
                                    ...
                                    if (attacker.NobilityRank == NobilityRank.King) {
                                        damage = (uint)(damage * 1.4);
                                    }

                                    if (attacker.NobilityRank == NobilityRank.Prince) {
                                        damage = (uint)(damage * 1.0);
                                    }

                                    if (attacker.NobilityRank == NobilityRank.Duke) {
                                        damage = (uint)(damage * 0.8);
                                    }

                                    if (attacker.NobilityRank == NobilityRank.Earl) {
                                        damage = (uint)(damage * 0.5);
                                    }
                                        ...
                                        if (attacker.ContainsFlag2(Update.Flags2.ChainBoltActive)) {
                                            ...
                                            if (attacked != null) {
                                                if (Kernel.GetDistance(attacker.X, attacker.Y, attacked.X,
                                                    ...
                                                    if (CanAttack(attacker, attacked, spell,
                                                            attack.AttackType == Attack.Melee)) {
                                                        var damage = Calculate.Magic(attacker, attacked, spell,
                                                            ref attack);
                                                        ReceiveAttack(attacker, attacked, attack, ref damage, spell);
                                                        suse.AddTarget(attacked, damage, attack);
                                                    }
                                            ...
                                            var Array = attacker.Owner.Screen.Objects;
                                            var closestTarget = findClosestTarget(attacked, attacked.X, attacked.Y,
                                                Array);
                                        ...
                                        if (attacked != null) {
                                            ...
                                            if (attackedsob != null) {
                                                if (Kernel.GetDistance(attacker.X, attacker.Y, attackedsob.X,
                                                    ...
                                                    if (CanAttack(attacker, attackedsob, spell)) {
                                                        PrepareSpell(spell, attacker.Owner);
                                                        var damage = Calculate.Magic(attacker, attackedsob, spell,
                                                            ref attack);
                                                        if (Kernel.Rate(spell.Percent)) {
                                                            ReceiveAttack(attacker, attackedsob, attack, damage, spell);
                                ...
                                case 11980: {
                                    if (CanUseSpell(spell, attacker.Owner)) {
                                        if (Time32.Now > attacker.MortalWoundStamp.AddMilliseconds(2000)) {
                                            ...
                                            byte dist = (byte)spell.Distance;
                                            var Array = attacker.Owner.Screen.Objects;
                                            InLineAlgorithm algo = new InLineAlgorithm(attacker.X, X, attacker.Y, Y,
                                                dist,
                                                InLineAlgorithm.Algorithm.DDA);
                                            // X = attacker.X;
                                            //  Y = attacker.Y;
                                            ...
                                            foreach (IMapObject _obj in Array) {
                                                ...
                                                if (hit) {
                                                    if (_obj.MapObjType == MapObjectType.Monster) {
                                                        ...
                                                        if (CanAttack(attacker, attacked, spell,
                                                                attack.AttackType == Attack.Melee)) {
                                                            var damage = Calculate.Melee(attacker, attacked,
                                                                ref attack);
                                                            damage = (uint)(damage * 0.8);
                                                            ReceiveAttack(attacker, attacked, attack, ref damage,
                                                                spell);
                                                        ...
                                                        attackedsob = _obj as SobNpcSpawn;
                                                        if (CanAttack(attacker, attackedsob, spell)) {
                                                            var damage = Calculate.Melee(attacker, attackedsob,
                                                                ref attack);
                                                            ReceiveAttack(attacker, attackedsob, attack, damage, spell);
                                        ...
                                        if (Kernel.GetDistance(attacker.X, attacker.Y, Xx, Yx) <= 5) {
                                            ...
                                            if (attackedsob == null) {
                                                if (CanAttack(attacker, attacked, spell,
                                                    ...
                                                    var dist = Kernel.GetDistance(attacker.X, attacker.Y, attacked.X,
                                                        attacked.Y);
                                                    suse.Effect1 = attack.Effect1;
                                                    damage = (uint)(damage * 1.75);
                                                    ReceiveAttack(attacker, attacked, attack, ref damage, spell);
                                                    ...
                                                    if (attacker.Owner.Spells.ContainsKey(11230) && !attacked.Dead) {
                                                        ...
                                                        if (spellz != null) {
                                                            if (Kernel.Rate(Percent)) {
                                                                ...
                                                                ssuse.SpellLevel = spellz.Level;
                                                                damage = Calculate.Melee(attacker, attacked,
                                                                    ref attack); // poner algo mas que el ataque de arriba
                                                                // damage = (uint)(damage * spell.Power / 1000);
                                                                ssuse.AddTarget(attacked,
                                                                    new SpellUse.DamageClass().Damage = damage, attack);
                                                                ReceiveAttack(attacker, attacked, attack, ref damage,
                                                                    spellz);
                                        ...
                                        foreach (IMapObject _obj in Array) {
                                            ...
                                            if (hit) {
                                                if (_obj.MapObjType == MapObjectType.Monster ||
                                                    ...
                                                    if (CanAttack(attacker, attacked, spell,
                                                            attack.AttackType == Attack.Melee)) {
                                                        var damage = Calculate.Ranged(attacker, attacked, ref attack) /
                                                                     2;
                                                        ReceiveAttack(attacker, attacked, attack, ref damage, spell);
                                ...
                                case 12200: {
                                    if (CanUseSpell(spell, attacker.Owner)) {
                                        ...
                                        if (attacker.OnDragonSwing) {
                                            ...
                                            Update upgrade = new Update(true);
                                            upgrade.UID = attacker.UID;
                                            upgrade.Append(Update.DragonSwing, (uint)spell.Duration, (uint)spell.Status,
                                ...
                                default: {
                                    if (attacker.Owner.Account.State == AccountTable.AccountState.GM)
                                        attacker.Owner.Send(new Message("Unknown Skill : " + spellID, Color.CadetBlue,
                                            Message.Talk));
                                    break;
                                }
                            }
                            ...
                            if (attacker.MapID == 1039) {
                                ...
                                if (spell.NextSpellID != 0) {
                                    ...
                                    var xspell =
                                        GetWeaponSpell(
                                            spell); //Database.SpellTable.WeaponSpells.Values.Where(p => p.Contains(spell.ID));
                        ...
                        if (attacker.AttackPacket == null) {
                            ...
                            if (attacker.Owner.Spells.ContainsKey(11960)) {
                                if (spell.ID == 9876 || spell.ID == 6002 || spell.ID == 10315 || spell.ID == 10311 ||
                                    spell.ID == 10313 ||
                                    spell.ID == 6003 || spell.ID == 10405 || spell.ID == 30000 || spell.ID == 10310 ||
                                    spell.ID == 3050 ||
        ...
        public static void HandleAura(Entity attacker, SpellInformation spell) {
            ulong statusFlag = 0;
            ulong statusFlag2 = 0;
        ...
        public static List<IMapObject> GetObjects(UInt16 ox, UInt16 oy, GameState c) {
            UInt16 x, y;
            x = c.Entity.X;
            y = c.Entity.Y;
        ...
        public static void ReceiveAttack(Entity attacker, Entity attacked, Attack attack, ref uint damage,
            ...
            if (!(attacked.Name.Contains("Guard") && attacked.EntityFlag == EntityFlag.Monster))
                if (attacker.EntityFlag == EntityFlag.Player && attacked.EntityFlag != EntityFlag.Player &&
                    ...
                    if (damage > attacked.Hitpoints) {
                        attacker.Owner.IncreaseExperience(
                            Calculate.CalculateExpBonus(attacker.Level, attacked.Level,
                                Math.Min(damage, attacked.Hitpoints)), true);
                        if (spell != null)
                            attacker.Owner.IncreaseSpellExperience(
                                (uint)Calculate.CalculateExpBonus(attacker.Level, attacked.Level,
                                    Math.Min(damage, attacked.Hitpoints)), spell.ID);
                    }
                    else {
                        attacker.Owner.IncreaseExperience(
                            Calculate.CalculateExpBonus(attacker.Level, attacked.Level, damage), true);
                        if (spell != null)
                            attacker.Owner.IncreaseSpellExperience(
                                (uint)Calculate.CalculateExpBonus(attacker.Level, attacked.Level, damage), spell.ID);
                    }
                }
                    ...
                    if (attacked.Owner != null && attacker.Owner != null) {
                        if (attacked.Owner.Team != null && attacker.Owner.Team != null) {
                            if (attacker.Owner.Team.EliteFighterStats != null &&
                                attacked.Owner.Team.EliteFighterStats != null) {
                                if (attacker.Owner.Team.EliteMatch != null) {
                                    if (!attacked.Owner.Team.Alive) {
                                        ...
                                        attacker.Owner.Team.SendMesageTeam(
                                            attacker.Owner.Team.EliteMatch.CreateUpdate().ToArray(), 0);
                                        attacked.Owner.Team.SendMesageTeam(
                                            attacker.Owner.Team.EliteMatch.CreateUpdate().ToArray(), 0);
                                    }
                                }
                            }
        ...
        public static void ReceiveAttack(Entity attacker, SobNpcSpawn attacked, Attack attack, uint damage,
            ...
            if (attacker.MapID == 1002) {
                if (attacked.UID == 61010) {
                    attacked.Die(attacker);
                    {
                        ...
                        attacker.Send(new Message("You received 50 CPs! for Hit Stake " + Kernel.GamePool.Count,
                            Message.TopLeft));
                    }
                }
            }
            ...
            if (attacker.MapID == 10380) {
                if (attacked.UID == 8100) {
                    if (SuperGuildWar.PoleKeeper == attacker.Owner.Guild)
                        return;
                    if (attacked.Hitpoints <= damage)
                        attacked.Hitpoints = 0;
                    SuperGuildWar.AddScore(damage, attacker.Owner.Guild);
                }
            }
        ...
        public static bool CanUseSpell(SpellInformation spell, GameState client) {
            if (GameConstants.SSFB.Contains(client.Entity.MapID)) {
                ...
                {
                    client.Send(new Message("You can't use any skills here Except FB And SS  ", Color.Red, 0x7dc));
                    return false;
                }
            }
        ...
        public static void PrepareSpell(SpellInformation spell, GameState client) {
            if (spell.NeedXP == 1)
                client.Entity.RemoveFlag(Update.Flags.XPList);
        ...
        public static bool CanAttack(Entity attacker, SobNpcSpawn attacked, SpellInformation spell) {
            ...
            if (GuildScoreWar.Map != null) {
                if (attacker.MapID == GuildScoreWar.Map.ID) {
                    if (attacker.GuildID == 0 || !GuildScoreWar.IsWar) {
            ...
            if (attacker.MapID == 10380) {
                if (attacker.GuildID == 0 || !SuperGuildWar.IsWar) {
                    if (attacked.UID == 8100) {
                        return false;
                    }
                }

                if (SuperGuildWar.PoleKeeper != null) {
                    if (SuperGuildWar.PoleKeeper == attacker.Owner.Guild) {
                        if (attacked.UID == 8100) {
                            return false;
                        }
            ...
            if (attacker.MapID == 2071) {
                if (attacker.GuildID == 0 || !EliteGuildWar.IsWar) {
                    if (EliteGuildWar.Poles != null)
                        if (attacked.UID == EliteGuildWar.Poles.UID)
                            return false;
                }
        ...
        public static void cheakteam(GameState client, MTA.Npcs dail) {
            if (client.Team == null)
                    ...
                    if (TeamCount == lastcheak) {
                        ...
                        Kernel.SendWorldMessage(
                            new Message(
                                ...
                                " Team Has Won LAst Team Standing and all Team Member Got " + won + " ConquerPoints",
                                Color.Red, 2011), Program.Values);
                    }
                    else {
        ...
        public static bool CanAttack(Entity attacker, Entity attacked, SpellInformation spell, bool melee) {
            ...
            if (attacker.MapID == 1507) {
                if (!ClanWarArena.Tournaments[4].Open) {
            ...
            if (attacker.EntityFlag == EntityFlag.Player && attacked.EntityFlag == EntityFlag.Player)
                if ((attacker.Owner.InQualifier() && attacked.Owner.IsWatching()) ||
                    (attacked.Owner.InQualifier() && attacker.Owner.IsWatching()))
                    return false;
            if (attacker.EntityFlag == EntityFlag.Player)
                ...
                if (attacked.Companion) {
                    if (GameConstants.IsPKForbidden(attacker.Owner.Map.ID, attacker.Owner.Map)) {
                        ...
                        if (attacker.PKMode != Enums.PkMode.PK &&
                            ...
                            attacker.AddFlag(Update.Flags.FlashingName);
        ...
        public static void CheckForExtraWeaponPowers(GameState client, Entity attacked) {
            #region Right Hand
        ...
        public void CheckForSuperGems(GameState client) {
            for (uint i = 1; i < 12; i++) {
                if (i != 7) {
        ...
        public class InteractionRequest {
            public InteractionRequest(Attack attack, Entity a_client) {
        ...
        public class InteractionAccept {
            public InteractionAccept(Attack attack, Entity a_client) {
                GameState client = a_client.Owner;
        ...
        public class InteractionRefuse {
            public InteractionRefuse(Attack attack, Entity a_client) {
                GameState client = a_client.Owner;
