...
namespace MTA.MaTrix {
    public class MatrixMob {
        public static SafeDictionary<uint, SafeDictionary<uint, MatrixMob>> Mobs =
            new SafeDictionary<uint, SafeDictionary<uint, MatrixMob>>();
            ...
            if (Kernel.Maps.ContainsKey(MapID)) {
                ...
                if (MonsterInformation.MonsterInformations.ContainsKey(monsterID)) {
                    ...
                    Kernel.SendWorldMessage(new Message(
                        entity.Name + " has shown up in " + ((Enums.Maps)entity.MapID).ToString() + " (" + entity.X +
                        ", " + entity.Y + ")", Color.Red, Message.BroadcastMessage));
                    Console.WriteLine(entity.Name + " has shown up in " + ((Enums.Maps)entity.MapID).ToString() + " (" +
                                      entity.X + ", " + entity.Y + ")");
