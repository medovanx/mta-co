...
namespace MTA.Database
{
    public static class EntityTable
    {
        ...
        public static void SetFlowers(Client.GameState client)
        {
            ...
            if (IsGirl(client.Entity.Body))
            {
                ...
                else
                {
                    ...
                    if (Game.Features.Flowers.Database.FlowerTable.Exists(client.Entity.UID))
                    {
                        // Record exists but LoadByEntityId returned null - retry loading
                        var retryRecord = Game.Features.Flowers.Database.FlowerTable.LoadByEntityId(client.Entity.UID);
                        if (retryRecord != null)
                        {
                            client.Entity.Flowers.RedRoses = retryRecord.RedRoses;
                            client.Entity.Flowers.RedRosesToday = retryRecord.RedRosesToday;
                            client.Entity.Flowers.Lilies = retryRecord.Lilies;
                            client.Entity.Flowers.Lilies2day = retryRecord.LiliesToday;
                            client.Entity.Flowers.Orchids = retryRecord.Orchids;
                            client.Entity.Flowers.OrchidsToday = retryRecord.OrchidsToday;
                            client.Entity.Flowers.Tulips = retryRecord.Tulips;
                            client.Entity.Flowers.TulipsToday = retryRecord.TulipsToday;
                            client.Entity.Flowers.SendDay = retryRecord.SendDay;
                            client.Entity.Flowers.AFlower = retryRecord.AFlower;
                            try
                            {
                                client.Entity.Flowers.LastFlowerSent = System.DateTime.FromBinary(retryRecord.LastFlowerSent);
                            }
                            catch
                            {
                                client.Entity.Flowers.LastFlowerSent = System.DateTime.Now.Subtract(System.TimeSpan.FromDays(1));
            ...
            // For boys, just initialize with default values (not saved to database)
        }

        public static void SetKisses(Client.GameState client)
        {
            // Only initialize kisses for boys
            ...
            {
                record = Game.Features.Kisses.Database.KissTable.LoadByEntityId(client.Entity.UID);
            }
            ...
            if (record != null)
            {
                ...
                // Add to ranking lists if they have any kiss data
                if (client.Entity.Kisses.Kisses2 > 0)
                    MTA.Game.Features.Kisses.Kisses.CalculateRankKisses(client.Entity.Kisses);
                if (client.Entity.Kisses.Letters1 > 0)
                    MTA.Game.Features.Kisses.Kisses.CalculateRankLetters(client.Entity.Kisses);
                if (client.Entity.Kisses.Wine > 0)
                    MTA.Game.Features.Kisses.Kisses.CalculateRankWine(client.Entity.Kisses);
                if (client.Entity.Kisses.Jades > 0)
                    MTA.Game.Features.Kisses.Kisses.CalculateRankJades(client.Entity.Kisses);
            }
            else
            {
                // Only create new record if it truly doesn't exist
                if (!Game.Features.Kisses.Database.KissTable.Exists(client.Entity.UID))
                {
                ...
                else
                {
                    ...
                    if (reader.Read())
                    {
                        ...
                        client.Entity.Kisses.Kisses2day = reader.ReadUInt32("kisses_today");
                        client.Entity.Kisses.Letters1 = reader.ReadUInt32("letters");
                        client.Entity.Kisses.LetterToday1 = reader.ReadUInt32("letters_today");
                        client.Entity.Kisses.Wine = reader.ReadUInt32("wine");
                        client.Entity.Kisses.Wine2day = reader.ReadUInt32("wine_today");
                        client.Entity.Kisses.Jades = reader.ReadUInt32("jades");
                        client.Entity.Kisses.Jades2day = reader.ReadUInt32("jades_today");
                        try
                        {
                            client.Entity.Kisses.LastKissesSent = System.DateTime.FromBinary(reader.ReadInt64("last_kiss_sent"));
                        }
                        catch
                        {
                            client.Entity.Kisses.LastKissesSent = System.DateTime.Now.Subtract(System.TimeSpan.FromDays(1));
        ...
        /// <summary>
        ///     Migrates a player's data when they change gender
        ///     Deletes from old table and creates new record in correct table
        /// </summary>
        public static void MigrateGenderChange(Client.GameState client, ushort oldBody) {
            // Delete from old table
            if (IsGirl(oldBody)) {
                // Was a girl, delete from flowers table
                Game.Features.Flowers.Database.FlowerTable.DeleteByEntityId(client.Entity.UID);
                client.Entity.Flowers = null;
            }
            else if (IsBoy(oldBody)) {
                // Was a boy, delete from kisses table
                Game.Features.Kisses.Database.KissTable.DeleteByEntityId(client.Entity.UID);
                client.Entity.Kisses = null;
            }
            ...
            else if (IsBoy(client.Entity.Body)) {
                SetKisses(client);
            }
        }
