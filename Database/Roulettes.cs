...
namespace MTA.Database {
    public class Roulettes {
        ...
        public unsafe class RouletteTable : NumberInformation {
            ...
            public ConcurrentDictionary<uint, GameState> ClientsWatch;
            ...
            public void AddWatch(GameState client) {
                ...
                if (client.PlayRouletteUID != 0) {
                    client.Send(new Message("You are play another table. Please quit before you spectate this one.",
                        Message.System));
                    return;
                }
                ...
                if (client.WatchRoulette != 0) {
                    client.Send(new Message(
                        "You are spectating another table. Please quit before you spectate this one.", Message.System));
                    return;
                }
                ...
                if (!ClientsWatch.ContainsKey(client.Entity.UID)) {
                    ...
                    GuiPacket.FinalizePacket(client,
                        RegistredPlayers.Values.Where(p => p.Owner.Entity.UID != client.Entity.UID).ToArray());
                }
            }
            ...
            public unsafe void AddPlayer(GameState client) {
                ...
                if (client.PlayRouletteUID != 0) {
                    client.Send(new Message("You are play another table. Please quit before you playing this one.",
                        Message.System)); //, Message.MsgColor.red);
                    return;
                }
                ...
                if (!RegistredPlayers.ContainsKey(client.Entity.UID)) {
                    ...
                    client.OnDisconnect = p => {
                        ...
                        if (p.PlayRouletteUID != 0) {
                            if (RoulettesPoll.TryGetValue(p.PlayRouletteUID, out table)) {
                                table.RemovePlayer(p);
                            }
                        }
                        else if (p.WatchRoulette != 0) {
                            if (RoulettesPoll.TryGetValue(p.WatchRoulette, out table)) {
            ...
            public void ShareTableBetting(GameState client) {
                foreach (var use in RegistredPlayers.Values) {
                    if (use.MyLuckExtra.Count != 0 || use.MyLuckNumber.Count != 0) {
                        MsgRouletteShareBetting BetPacket =
                            new MsgRouletteShareBetting(use.MyLuckNumber.Count + use.MyLuckExtra.Count, use.Color);
            ...
            public class Member {
                ...
                public ConcurrentDictionary<byte, MsgRouletteCheck.Item> MyLuckNumber =
                    new ConcurrentDictionary<byte, MsgRouletteCheck.Item>();
                ...
                public void ShareBetting(RouletteTable table) {
                    MsgRouletteShareBetting BetPacket =
                        new MsgRouletteShareBetting(MyLuckNumber.Count + MyLuckExtra.Count, Color);
